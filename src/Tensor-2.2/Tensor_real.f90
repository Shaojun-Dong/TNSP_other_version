...
!		size(T%Tensor_Data).
!			2. array=T, will not allocate the array. But CopyDTensor(array,T) will allocate array
...
!*********		define data type	***************
	type  DTensor
		...
		logical,private :: flag=.false. !if flag=true ,it means there are data in DTensor
	end type DTensor
...
!one can use subroutine setTensorName(T,Tensorname,nameID) to set a name to the Tensor
	...
	interface Dgenerate
		...
		module procedure Dgenerate_dim
	...
	interface setDTensorName
		...
		module procedure setDTensorName5
		module procedure setDTensorName6
		module procedure setDTensorName7
		module procedure setDTensorName8
		module procedure setDTensorName9
	end interface
	interface Dmodify
		module procedure Dmodifylink
		module procedure Dmodifylist
		module procedure DmodifyTen_all!Ten%Tensor_Data(1:Ten%TotalData)=Allval,call modify(T,Allval)
		module procedure DmodifyTen_val!T(i)=value or T(i,j)=value, if the rank<=4 ,the speed is the same as array,The program will check the input
		                              !call modify(T,dimen,val),dimen=(/ith/) or dimen=(/ith,ith/), dimen=(/ith,jth,kth,../)
		module procedure DmodifyTen_val1!call modify(T,ith,val)
		module procedure DmodifyTen_array_val1!T(i1:i2)=value(:),the speed is the same as array
		                                     !call modify(T,dimen,array),dimen=(/i1,i2/)
		module procedure DmodifyTen_array_val2!T(i1:i2,j1:j2)=value(:,:),the speed is the same as array
		                                     !call modify(T,dimen1,dimen2,array),dimen1=(/i1,i2/),dimen3=(/j1,j2/)   
		module procedure DmodifyTen_array_val2_2    !T(i1:i2,j1:j2)=value(:),the speed is the same as array     
		module procedure DmodifyTen_array_val3!T(i1:i2,j1:j2,l1:l2)=value(:,:,:),the speed is the same as array
		                         !call modify(T,dimen1,dimen2,dimen3,array),dimen1=(/i1,i2/),dimen1=(/j1,j2/),dimen3=(/l1,l2/)  
		module procedure DmodifyTen_array_val3_2!T(i1:i2,j1:j2,l1:l2)=value(:),the speed is the same as array
		module procedure DmodifyTen_array_val4!T(i1:i2,j1:j2,k1:k2,l1:l2)=value(:,:,:,:),the speed is the same as array
		                         !call modify(T,dimen1,dimen2,dimen3,array),dimen1=(/i1,i2/),dimen1=(/j1,j2/),dimen3=(/l1,l2/)  
		module procedure DmodifyTen_array_val4_2!T(i1:i2,j1:j2,k1:k2,l1:l2)=value(:),the speed is the same as array
		module procedure DmodifyTen_Tensor_val1!T(i1:i2)=T2(:),the speed is the same as array,T2 is a Tensor
		module procedure DmodifyTen_Tensor_val2!T(i1:i2,j1:j2)=T2(:,:),the speed is the same as array,T2 is a Tensor
		module procedure DmodifyTen_Tensor_val2_2!T(i1:i2,j1:j2)=T2,the speed is the same as array,T2 is a Tensor
		module procedure DmodifyTen_Tensor_val3!T(i1:i2,j1:j2,l1:l2)=T2(:,:,:),the speed is the same as array,T2 is a Tensor
		module procedure DmodifyTen_Tensor_val3_2!T(i1:i2,j1:j2,l1:l2)=T2,the speed is the same as array,T2 is a Tensor
		module procedure DmodifyTen_Tensor_val4!T(i1:i2,j1:j2,k1:k2,l1:l2)=T2(:,:,:,:),the speed is the same as array,T2 is a Tensor
		module procedure DmodifyTen_Tensor_val4_2!T(i1:i2,j1:j2,k1:k2,l1:l2)=T2,the speed is the same as array,T2 is a Tensor
		module procedure DmodifyTen_Tensor_all!Ten%Tensor_Data(i1:i2)=T2%Tensor_Data
	end interface
!************************************************************	
...
!	 DTensor_Data of DTensor.
	interface DdimOperation
		...
		module procedure DdimOperation2
		module procedure DdimOperation3
	end interface
...
!										!example,T=one_Ten((/2,3,4/)), then T(i,i,i)=1d0,other T=0d0
	...
	interface DzeroTen!generate a Tensor with all element being 0d0
		module procedure DzeroTen1
		module procedure DzeroTen2
	end interface
!******************  Tenproduct  *********************
...
! 	input Tensor should be in its original dimenison,there is no contract on it
	interface DTenproduct
		...
		module procedure DTenproduct_noName2
		module procedure DTenproduct_Name!In put dimension as character
		module procedure DTenproduct_Name2
		module procedure DTenproduct_IntName!In put dimension as character
		module procedure DTenproduct_IntName2
		module procedure DTenproduct_name_int
		module procedure DTenproduct_intname_int
		module procedure DTenproduct_name_int2
		module procedure DTenproduct_intname_int2
		module procedure DTenproduct_int_name
		module procedure DTenproduct_int_intname
		module procedure DTenproduct_int_name2
		module procedure DTenproduct_int_intname2
...
!  otherwhile ,create a new link	and that add to the list
	...
	interface DTenNameorder
		module procedure DTenNameorder1
		module procedure DTenNameorder2
		module procedure DTenNameorder3
		module procedure DTenNameorder4
	end interface
...
!		the function productDTensor add at 2013.10.28
	interface operator(*)
		...
		module procedure Dmultiply_numberReal4
		module procedure Dmultiply_numberReal4_
	end interface
...
!******************************************************************************************************
	...
	interface operator(.subDim.)
		...
		module procedure DgetTenSubDim_name
	...
	interface operator(.i.)
		...
		module procedure DElement!output the data in the DTensor of Tim=(i,j),if rank<=3, the speed is the same as array,The program will check the input
	...
	interface operator(.iiname.)!the index name
		module procedure DoutDimensionIntName
		module procedure DoutAllDimensionIntName
	end interface
	...
	interface operator(.Tiname.)!the Tensor name
		module procedure DoutTensorIntName
		module procedure DoutAllTensorIntName
	end interface
...
!allocate Tensor according to the dimen		
	...
	interface copyDTensor
		module procedure   copyDTensortoDTensor
		module procedure Dcopy_dim1!Vec=T ,Vec is a vector and T are DTensor,vec=T%DTensor_Data,used for any case,Mat will be allocate
		module procedure Dcopy_dim2!Mat=T ,Mat is a matrix and T are DTensor,Mat=T%DTensor_Data,used only rank=2,Mat will be allocate
		module procedure Dcopy_dim3!Mat=T ,Mat is a rank=3 and T are DTensor,Mat=T%DTensor_Data,used only rank=3,Mat will be allocate
	end interface
	interface assignment(=)
		module procedure DassignmentTen!T1=T2 ,both T1 and T2 are DTensor
		module procedure copyDTensordim1!Vec=T ,Vec is a vector and T are Tensor,vec=T%Tensor_Data,used for any case,Vec should be allocated
		module procedure copyDTensordim2!Mat=T ,Mat is a matrix and T are Tensor,Mat=T%Tensor_Data,used only rank=2,Mat should be allocated
		module procedure copyDTensordim3!Mat=T ,Mat is a rank=3 and T are Tensor,Mat=T%Tensor_Data,used only rank=3,Mat should be allocated
		module procedure DassignmentTen3!T=vec or matrix
		module procedure DassignmentTen2
		module procedure DassignmentTen1
		module procedure DassignmentTenNumReal
...
!
!		emptyDTensor: set Tensor to a emptyTensor, but do not deallocate memery
!
...
!
!		MPI function:
!			sent_DTensor(Ten1,Ten2,ID1,ID2,ierr,MPI_COMM):send the data of Ten1 in ID1 to Ten2 in ID2
!			BCAST_DTensor(Ten1,ID,ierr,MPI_COMM):BCAST The Ten1 in ID to every cpus
!			sent_DTensorlink(link1,link2,ID1,ID2,ierr,MPI_COMM):Send TensorLink,form link1 in ID1 to link2 in ID2
!			BCAST_DTensorlink(link1,ID,ierr,MPI_COMM):BCAST The link1 in ID to every CPUs
!
!			example
	...
	subroutine DassignmentTen(T,T2)
		...
		if(T2%flag) then
			...
			if(allocated(T%DTensor_Data)) then
				lengthold=size(T%DTensor_Data)
				if(lengthold.lt.length) then!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!If old data longer than new,do not allocate
...
!The output Tensor should be allocated
	subroutine copyDTensortoDTensor(T,T2)
		type(DTensor),intent(inout) ::T
		type(DTensor),intent(in) :: T2
		integer::length
		...
		T%TotalData=T2%TotalData
		T%flag=T2%flag
		call dcopy(T%TotalData,T2%DTensor_Data,1,T%DTensor_Data,1)
		return
	end subroutine
!Do not allocate the Tensor data
...
!allocate Tensor according to the dimen
	...
	!	allocate(T%DTensor_Data(T%totalData))
		call allocateCheck(T%DTensor_Data,T%totalData)
		T%flag=.true.
		return
	end subroutine
...
!		allocate(T%DTensor_Data(T%totalData))
	...
	subroutine DassignmentTenNumReal(T,num)
		type(DTensor),intent(inout)::T
...
!***********	DstoreTenData  **************
	subroutine DstoreTenData_dim1(T,inData)
		...
		length=size(inData)
		if(allocated(T%DTensor_data))then
			if(length.gt.size(T%DTensor_Data)) then!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!If old data longer than new,do not allocate
				deallocate(T%DTensor_Data)
				allocate(T%DTensor_Data(length))
			end if
		else
...
!		generate a DTensor with random number
	type(DTensor) function Dgenerate_NoName(Tdim,region) result (T)
		...
		do i=1,totalData
			DTensor_data(i)=(randomnumber()*delmum)+minnum
		end do
		TenDim=Tdim
	...
	type(DTensor) function Dgenerate_dim(Tdim,region) result (T)
		...
		do i=1,T%totalData
			T%DTensor_data(i)=(randomnumber()*delmum)+minnum
		end do
		return
	end function
...
!*********************** TensorName   **********************
	...
	function DoutTensorIntName(T,ith)
		integer,allocatable::DoutTensorIntName(:)
		type(DTensor),intent(in)::T
		integer,intent(in)::ith
		integer::TenNamelen
		TenNamelen=outTenNameLen()
		allocate(DoutTensorIntName(TenNamelen))
		DoutTensorIntName=outIntNameTen(T%TenDim,ith)
		return
	end function
	function DoutDimensionIntName(T,ith)
		integer,allocatable::DoutDimensionIntName(:)
		type(DTensor),intent(in)::T
		integer,intent(in)::ith
		integer::DimNamelen
		DimNamelen=outDimNameLen()
		allocate(DoutDimensionIntName(DimNamelen))
		DoutDimensionIntName=outIntNameDim(T%TenDim,ith)
		return
	end function
	...
	function DoutAllTensorIntName(T)
		integer,allocatable::DoutAllTensorIntName(:,:)
		type(DTensor),intent(in)::T
		integer::i,TenNamelen
		TenNamelen=outTenNameLen()
		allocate(DoutAllTensorIntName(T%rank,TenNamelen))
		do i=1,T%rank
			DoutAllTensorIntName(i,:)=outIntNameTen(T%TenDim,i)
		end do
		return
	end function
	function DoutAllDimensionintName(T)
		integer,allocatable::DoutAllDimensionintName(:,:)
		type(DTensor),intent(in)::T
		integer::i,DimNamelen
		DimNamelen=outDimNameLen()
		allocate(DoutAllDimensionintName(T%rank,DimNamelen))
		do i=1,T%rank
			DoutAllDimensionintName(i,:)=outIntNameDim(T%TenDim,i)
		end do
		return
	end function
	integer function DTenNameorder1(T,w)
		type(DTensor),intent(in) :: T
		character(len=*),intent(in)::w
		integer :: inde
		DTenNameorder1=Nameorder(T%TenDim,w)
		return
	end function
	function DTenNameorder2(T,w)
		integer,allocatable::DTenNameorder2(:)
		type(DTensor),intent(in) :: T
		character(len=*),intent(in)::w(2)
		integer :: inde
		allocate(DTenNameorder2(size(w)))
		DTenNameorder2=Nameorder(T%TenDim,w)
		return
	end function
	integer function DTenNameorder3(T,TensorName,DimenName)
		type(DTensor),intent(in) :: T
		integer,intent(in)::TensorName(:),DimenName(:)
		integer :: inde
		DTenNameorder3=Nameorder(T%TenDim,TensorName,DimenName)
		return
	end function
	function DTenNameorder4(T,TensorName,DimenName)
		type(DTensor),intent(in) :: T
		integer,allocatable::DTenNameorder4(:)
		integer,intent(in)::TensorName(:,:),DimenName(:,:)
		allocate(DTenNameorder4(size(TensorName,1)))		
		DTenNameorder4=Nameorder(T%TenDim,TensorName,DimenName)
		return
	end function
...
!		generate a Tensor with all element is 0d0
	type(DTensor) function DzeroTen1(Tdim) result (T)
		integer,intent(in) :: Tdim(:)
		integer :: rank,totalData,i
		rank=size(Tdim)
		totalData=product(Tdim)
...
!*********************  zeroTen *********************
!		generate a Tensor with all element is 0d0
	type(DTensor) function DzeroTen2(Tdim) result (T)
		type(Dimension),intent(in) :: Tdim
		call allocateDTensor(T,Tdim)
		T%DTensor_data=0D0
		return
...
!*********************  get DTensor dimenison *********************
	...
	type(Dimension) function DgetTenSubDim_name(T,w)
		type(DTensor),intent(in)::T
		CHARACTER(len=*),intent(in)::w
		integer::inde
		inde=DTenNameorder(T,w)
		DgetTenSubDim_name=T%TenDim.sub.inde
		return
	end function
!********************* print_DTensor *********************       
	subroutine DTprint1(T,words)
		type(DTensor),intent(in) :: T
		...
		if(T%flag) then
			...
			write(*,*) T%DTensor_data(1:T%totalData)
...
!********************* print_dim *********************            
	...
	subroutine DTprint2(T,printtype,words)
		type(DTensor),intent(in) :: T
		...
		if(T%flag) then
			...
			case (1)
				write(*,*) T%DTensor_data(1:T%totalData)
			case (2)
				write(*,'(99999F12.8)') T%DTensor_data(1:T%totalData)
			case (3)
				write(*,'(99999F10.2)') T%DTensor_data(1:T%totalData)
			end 	select
			call Dprint(T%TenDim)
			write(*,*) "***end***"
			write(*,*) ""
	...
	subroutine DTprint2_file(T,printtype,fileaddress,fileunit,replace)
		...
		if(T%flag) then
			...
			select case (printtype)
			case (1)
				write(fileunit,*) T%DTensor_data(1:T%totalData)
			case (2)
				write(fileunit,'(99999F12.8)') T%DTensor_data(1:T%totalData)
			case (3)
				write(fileunit,'(99999F10.2)') T%DTensor_data(1:T%totalData)
			end 	select
			!call Dprint(T%TenDim)
			write(fileunit,*) "***end***"
...
!********************* print_Matrix *********************
	...
	subroutine DTMprint2(T,printtype,words)
		type(DTensor),intent(in) :: T
		integer,intent(in)::printtype
		real*8,allocatable :: Tdata(:,:)
		real*8,allocatable :: Tdata3(:,:,:)
...
!cccccccccccccccc add          cccccccccccccccccc
	type(DTensor) function Dadd(T1,T2)
		...
		Dadd%Totaldata=T1%totaldata
		Dadd%flag=T1%flag
		Dadd%TenDim=T1%TenDim
		Dadd%rank=T1%rank
		allocate(Dadd%DTensor_Data(Dadd%Totaldata))
		call DCOPY(Dadd%Totaldata,T1%DTensor_Data,1,Dadd%DTensor_Data,1)
		call daxpy (Dadd%Totaldata, 1d0, T2%DTensor_Data,1,Dadd%DTensor_Data,1)
...
!cccccccccccccccc    Dminus       cccccccccccccccccc		
	type(DTensor) function Dminus(T1,T2)
		...
		Dminus%Totaldata=T1%totaldata
		Dminus%flag=T1%flag
		Dminus%TenDim=T1%TenDim
		Dminus%rank=T1%rank
		allocate(Dminus%DTensor_Data(Dminus%Totaldata))
		call DCOPY(Dminus%Totaldata,T1%DTensor_Data,1,Dminus%DTensor_Data,1)
		call daxpy (Dminus%Totaldata,-1d0, T2%DTensor_Data,1,Dminus%DTensor_Data,1)
		return
	end function
...
!cccccccccccccccc Ddivide          cccccccccccccccccc	
	type(DTensor) function Ddivide(T1,T2)
		...
		Ddivide=Dset(dim1,T1%DTensor_data(1:T1%Totaldata)/T2%DTensor_data(1:T2%Totaldata))
		return
	end function
!cccccccccccccccc Ddivide_number          cccccccccccccccccc	
	type(DTensor) function Ddivide_number(T1,num)
		...
		allocate(Ddivide_number%DTensor_Data(Ddivide_number%Totaldata))
		call DCOPY(Ddivide_number%Totaldata,T1%DTensor_Data,1,Ddivide_number%DTensor_Data,1)
		call DSCAL(Ddivide_number%Totaldata,1d0/num,Ddivide_number%DTensor_Data,1)
		Ddivide_number%flag=.true.
		return
	end function
	type(DTensor) function Ddivide_numberReal4(T1,num)result(Ddivide_number)
		type(DTensor),intent(in) :: T1
		...
		call DCOPY(Ddivide_number%Totaldata,T1%DTensor_Data,1,Ddivide_number%DTensor_Data,1)
		num_=1d0/num
		call DSCAL(Ddivide_number%Totaldata,num_,Ddivide_number%DTensor_Data,1)
		Ddivide_number%flag=.true.
		return
	end function
!cccccccccccccccc Dmultiply_DTensor          cccccccccccccccccc	
	type(DTensor) function Dmultiply_DTensor(T1,T2)
		...
		Dmultiply_DTensor=Dset(dim1,T1%DTensor_data(1:T1%Totaldata)*T2%DTensor_data(1:T2%Totaldata))
		return
	end function
!cccccccccccccccc Dmultiply_number          cccccccccccccccccc	
	type(DTensor) function Dmultiply_number(T1,num)
		...
		allocate(Dmultiply_number%DTensor_Data(Dmultiply_number%Totaldata))
		call DCOPY(Dmultiply_number%Totaldata,T1%DTensor_Data,1,Dmultiply_number%DTensor_Data,1)
		call DSCAL(Dmultiply_number%Totaldata,num,Dmultiply_number%DTensor_Data,1)
	...
	type(DTensor) function Dmultiply_number_(num,T1)
		...
		allocate(Dmultiply_number_%DTensor_Data(Dmultiply_number_%Totaldata))
		call DCOPY(Dmultiply_number_%Totaldata,T1%DTensor_Data,1,Dmultiply_number_%DTensor_Data,1)
		call DSCAL(Dmultiply_number_%Totaldata,num,Dmultiply_number_%DTensor_Data,1)
		Dmultiply_number_%flag=.true.
		return
	end function
	type(DTensor) function Dmultiply_numberReal4(T1,num)result(Dmultiply_number)
		type(DTensor),intent(in) :: T1
		real*4,intent(in) ::   num
		real*8::num_
		if(.not.T1%flag)then
		...
		call DCOPY(Dmultiply_number%Totaldata,T1%DTensor_Data,1,Dmultiply_number%DTensor_Data,1)
		num_=num
		call DSCAL(Dmultiply_number%Totaldata,num_,Dmultiply_number%DTensor_Data,1)
		Dmultiply_number%flag=.true.
		return
	end function
	type(DTensor) function Dmultiply_numberReal4_(num,T1)result(Dmultiply_number_)
		type(DTensor),intent(in) :: T1
		real*4,intent(in) ::   num
		real*8::num_
		if(.not.T1%flag)then
		...
		call DCOPY(Dmultiply_number_%Totaldata,T1%DTensor_Data,1,Dmultiply_number_%DTensor_Data,1)
		num_=num
		call DSCAL(Dmultiply_number_%Totaldata,num_,Dmultiply_number_%DTensor_Data,1)
		Dmultiply_number_%flag=.true.
		return
	end function
!**************************************************************
...
!  lenD is used in zcpoy, here lenD=LD1
	subroutine Dpermutation_rank3_data1(outdata,indata,LD1,LD2,LD3,lenD)
		integer,intent(in)::LD1,LD2,LD3,lenD
		real*8::outdata(LD1,LD3,LD2)
		real*8::indata(LD1,LD2,LD3)
		integer::i,j,k
		!do i=1,LD1
...
!  lenD is used in zcpoy, here lenD=LD1*LD3
	subroutine Dpermutation_rank3_data2(outdata,indata,LD1,LD2,LD3,lenD)
		integer,intent(in)::LD1,LD2,LD3,lenD
		real*8::outdata(LD3,LD2,LD1)
		real*8::indata(LD1,LD2,LD3)
		integer::i
		do i=1,LD2
			outdata(:,i,:)=transpose(indata(:,i,:))
...
!  lenD is used in zcpoy, here lenD=LD1*LD2
	subroutine Dpermutation_rank3_data3(outdata,indata,LD1,LD2,LD3,lenD)
		integer,intent(in)::LD1,LD2,LD3,lenD
		real*8::outdata(LD2,LD1,LD3)
		real*8::indata(LD1,LD2,LD3)
		integer::i
		do i=1,LD3
			outdata(:,:,i)=transpose(indata(:,:,i))
...
!  lenD is used in zcpoy, here lenD=LD1*LD2
	subroutine Dpermutation_rank2_data(outdata,indata,LD1,LD2,lenD)
		integer,intent(in)::LD1,LD2,lenD
		real*8::outdata(LD2,LD1)
		real*8::indata(LD1,LD2)
		outdata=transpose(indata)
		return
	end subroutine
!ccccccccccccccccc Dpermute_rank3   cccccccccccccccccc		
	type(DTensor) function Dpermute_rank3(T1,index_not_permute)result(Res)
		type(DTensor),intent(in) :: T1
		...
		if(index_not_permute.eq.1) then
			call allocateDTensor(Res, Dimpermute(T1%TenDim,(/1,3,2/)) )
			lenD=dimen(1)
			call Dpermutation_rank3_data1(Res%DTensor_Data,T1%DTensor_Data,dimen(1),dimen(2),dimen(3),lenD)
		end if
		if(index_not_permute.eq.2) then
			call allocateDTensor(Res, Dimpermute(T1%TenDim,(/3,2,1/)) )
			lenD=dimen(1)*dimen(3)
			call Dpermutation_rank3_data2(Res%DTensor_Data,T1%DTensor_Data,dimen(1),dimen(2),dimen(3),lenD)
		end if
		if(index_not_permute.eq.3) then
			call allocateDTensor(Res, Dimpermute(T1%TenDim,(/2,1,3/)) )
			lenD=dimen(1)*dimen(2)
			call Dpermutation_rank3_data3(Res%DTensor_Data,T1%DTensor_Data,dimen(1),dimen(2),dimen(3),lenD)
		end if
	 end function

!cccccccccccccccc Dpermute_rank2   cccccccccccccccccc			 
	type(DTensor) function dpermute_rank2 (T)result(Res)
		type(DTensor),intent(in) :: T
		integer::dimen(2)
		if(T%rank.eq.1) then
			Res=T
		...
		dimen=T%TenDim
		call allocateDTensor(Res, Dimpermute(T%TenDim,(/2,1/)) )
		call Dpermutation_rank2_data(Res%DTensor_Data,T%DTensor_Data,dimen(1),dimen(2),product(dimen))
	end function
!cccccccccccccccc Dpermutation   cccccccccccccccccc
	...
	subroutine Dpermutation_data3(T1data,index_not_permute,dimens,totaldata)
		...
		real*8,intent(inout) :: T1data(totaldata)
		type(Dimension),intent(inout) ::dimens
		real*8::oldData(totaldata)
		integer::dimen(3),lenD
		dimen=dimens
		call dcopy(totaldata,T1data,1,oldData,1)
		if(index_not_permute.eq.1) then
			lenD=dimen(1)
			call Dpermutation_rank3_data1(T1data,oldData,dimen(1),dimen(2),dimen(3),lenD)
			dimens=Dimpermute(dimens,(/1,3,2/))
			return
		end if
		if(index_not_permute.eq.2) then
			lenD=dimen(1)*dimen(3)
			call Dpermutation_rank3_data2(T1data,oldData,dimen(1),dimen(2),dimen(3),lenD)
			dimens=Dimpermute(dimens,(/3,2,1/))
			return
		end if
		if(index_not_permute.eq.3) then
			lenD=dimen(1)*dimen(2)
			call Dpermutation_rank3_data3(T1data,oldData,dimen(1),dimen(2),dimen(3),lenD)
	...
	subroutine Dpermutation_data2 (T1data,dimens,totaldata)
		...
		dimen=dimens
		call dcopy(totaldata,T1data,1,oldData,1)
		call Dpermutation_rank2_data(T1data,oldData,dimen(1),dimen(2),product(dimen))
	...
	subroutine Dpermutefo_data(Tdata,inde,dimen,totaldata)
		...
		if(inde.le.0) then
			write(*,*)"ERROR in function Dpermutefo_data"
			write(*,*)"index=",inde
			call error_stop()
		end if
		if(inde.eq.1) then
	...
	subroutine Dpermuteback_data(Tdata,inde,dimen,totaldata)
		...
		if(inde.le.0) then
			write(*,*)"ERROR in function Dpermuteback_data"
			write(*,*)"index=",inde
			call error_stop()
		end if
		if(inde.eq.1) then
...
!
	type(DTensor) function Dpermutefo(T,inde)
		...
		integer::oper(2,3)
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(Dpermutefo)"
			call error_stop()
		end if
		rank=DgetRank(T)
		if(inde.gt.rank) then
	...
	type(DTensor) function Dpermutefo_name(T,indechar)result(Dpermutefo)
		...
		integer::oper(2,3)
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(Dpermutefo_name)"
			call error_stop()
		end if
		rank=DgetRank(T)
		...
		if(inde.le.0) then
			write(*,*)"ERROR in function Dpermutefo_name,Can not find the name"
			write(*,*)"stop"
			call error_stop()
		end if
		if(inde.eq.1) then
...
!
	type(DTensor) function Dpermutefo_vec(T,vec_)
		...
		type(Dimension)::dimen		
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(Dpermutefo_vec)"
			call error_stop()
		end if
		rank=DgetRank(T)
		lenVec=size(vec_)
	...
	type(DTensor) function Dpermutefo_vec_name(T,indechar)result(Dpermutefo_vec)
		...
		type(Dimension)::dimen		
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(Dpermutefo_vec_name)"
			call error_stop()
		end if
		rank=DgetRank(T)
...
!
	type(DTensor) function Dpermuteback(T,inde)
		...
		integer::oper(2,3)
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(Dpermuteback)"
			call error_stop()
		end if
		rank=DgetRank(T)
		if(inde.gt.rank) then
		...
		if(inde.le.0) then
			write(*,*)"ERROR in function Dpermuteback,Can not find the name"
			write(*,*)"index",inde
			call error_stop()
		end if
		if(inde.eq.rank) then
	...
	type(DTensor) function Dpermuteback_name(T,indechar)result(Dpermuteback)
		...
		integer::oper(2,3)
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(Dpermuteback_name)"
			call error_stop()
		end if
		rank=DgetRank(T)
		inde=Nameorder(T%TenDim,indechar)
		...
		if(inde.le.0) then
			write(*,*)"ERROR in function Dpermuteback_name,Can not find the inde"
			write(*,*)"index",inde
			call error_stop()
		end if
		if(inde.eq.1) then
...
!
	type(DTensor) function DpermuteInde(T,inde)
		...
		integer::oper(2,3)
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(DpermuteInde)"
			call error_stop()
		end if
		rank=DgetRank(T)
		if(inde.gt.rank) then
		...
		if(inde.le.0) then
			write(*,*)"ERROR in function DpermuteInde"
			write(*,*)"index",inde
			call error_stop()
		end if
		if(inde.eq.1) then
	...
	type(DTensor) function DpermuteInde_name(T,w)result(DpermuteInde)
		type(DTensor),intent(in)::T
		character(len=*),intent(in)::w
		integer::inde
		integer::rank
...
!
	type(DTensor) function DpermutebackInde(T,inde)
		...
		integer::oper(2,3)
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(DpermutebackInde)"
			call error_stop()
		end if
		rank=DgetRank(T)
		if(inde.gt.rank) then
	...
	type(DTensor) function DpermutebackInde_name(T,w)result(DpermutebackInde)
		type(DTensor),intent(in)::T
		character(len=*),intent(in)::w
		integer::inde
		integer::rank
...
! 	input Tensor should be in its original dimenison,there is no contract on it
	...
	type(DTensor) function DTenproduct_noName2(T1_,i1,T2_,i2) result(T)
		type(DTensor),intent(in) :: T1_,T2_
		integer,intent(in) :: i1,i2
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
			write(*,*)"ERROR in Tenproduct"
	...
	type(DTensor) function DTenproduct_name2(T1_,name1,T2_,name2) result(T)
		type(DTensor),intent(in) :: T1_,T2_
		character(len=*),intent(in)::name1,name2
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
			write(*,*)"ERROR in Tenproduct"
	...
	type(DTensor) function DTenproduct_Intname(T1_,name1,Nimname1,T2_,name2,Nimname2) result(T)
		type(DTensor),intent(in) :: T1_,T2_
	...
	type(DTensor) function DTenproduct_Intname2(T1_,name1,Nimname1,T2_,name2,Nimname2) result(T)
		type(DTensor),intent(in) :: T1_,T2_
		integer,intent(in)::name1(:),name2(:),Nimname1(:),Nimname2(:)
		integer :: i1,i2
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
	...
	type(DTensor) function DTenproduct_int_name2(T1_,i1,T2_,name2) result(T)
		type(DTensor),intent(in) :: T1_,T2_
		character(len=*),intent(in)::name2
		integer,intent(in)::i1
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
	...
	type(DTensor) function DTenproduct_int_intname(T1_,i1,T2_,name2,DimName2) result(T)
		type(DTensor),intent(in) :: T1_,T2_
	...
	type(DTensor) function DTenproduct_int_intname2(T1_,i1,T2_,name2,DimName2) result(T)
		type(DTensor),intent(in) :: T1_,T2_
		integer,intent(in)::i1,name2(:),DimName2(:)
		integer :: i2
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
	...
	type(DTensor) function DTenproduct_name_int2(T1_,name1,T2_,i2) result(T)
		type(DTensor),intent(in) :: T1_,T2_
		character(len=*),intent(in)::name1
		integer,intent(in)::i2
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
	...
	type(DTensor) function DTenproduct_intname_int(T1_,name1,DimName1,T2_,i2) result(T)
		type(DTensor),intent(in) :: T1_,T2_
	...
	type(DTensor) function DTenproduct_intname_int2(T1_,name1,DimName1,T2_,i2) result(T)
		type(DTensor),intent(in) :: T1_,T2_
		integer,intent(in)::i2,name1(:),DimName1(:)
		integer :: i1
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
	...
	type(DTensor) function ProductDTensor (T1,T2)
		...
		select case (flag)
			...
			case (2)
				...
				if(T2n.eq.1)then![m]*[m,1]case
					ProductDTensor%DTensor_Data(1)=Ddot(T2m,T1%DTensor_Data,1,T2%DTensor_Data,1)
				else
		...
		case (3)
				...
				if(T1m.eq.1)then![1,m]*[m]case
					ProductDTensor%DTensor_Data(1)=Ddot(T1n,T1%DTensor_Data,1,T2%DTensor_Data,1)
				else
		...
		case (4)
				...
				if((T1m.eq.1).and.(T2n.eq.1))then![1,m] [m,1]
					...
					call DGEMV('T',T2m,T2n,dcmplx(1d0,0d0),T2%DTensor_Data,T2m,T1%DTensor_Data,&
							1,dcmplx(0d0,0d0),ProductDTensor%DTensor_Data,1)
				else if((T1m.ne.1).and.(T2n.eq.1))then![m,n] [n,1]
					call DGEMV('N',T1m,T1n,dcmplx(1d0,0d0),T1%DTensor_Data,T1m,T2%DTensor_Data,&
						1,dcmplx(0d0,0d0),ProductDTensor%DTensor_Data,1)
				else
...
!		combine two index of the DTensor,which is con_index and con_index+1
	type(DTensor) function Dcontract(T1,con_index)
		...
		call DstoreTenData(Dcontract,T1%DTensor_Data(1:T1%totalData))
...
!		if index2 larger than rank,the function will Dcontract the index of index1 -> rank
	type(DTensor) function Dcontracts(T1,index1,index2)
		...
		call DstoreTenData(Dcontracts,T1%DTensor_Data(1:T1%totalData))
	...
	type(DTensor) function Dcontractsv(T1,vector)
		...
		call DstoreTenData(Dcontractsv,T1%DTensor_Data(1:T1%totalData))
...
!		if inde larger than rank ,the function will return no change	
	type(DTensor) function Ddecompose(T1,de_index,inde)
		...
		call DstoreTenData(Ddecompose,T1%DTensor_Data(1:T1%totalData))
	...
	type(DTensor) function Ddecomposev(T1,vector)
		...
		inde=vector(2)
		newDim=Dimdecompose(T1%TenDim,de_index,inde)
		call DstoreTenData(Ddecomposev,T1%DTensor_Data(1:T1%totalData))
	...
	type(DTensor) function Ddecompose1(T1,de_index)
		...
		call DstoreTenData(Ddecompose1,T1%DTensor_Data(1:T1%totalData))
	...
	type(DTensor) function DdecomposeAll(T1)
		...
		call DstoreTenData(DdecomposeAll,T1%DTensor_Data(1:T1%totalData))
...
!		other DElement of every row is input parameter of the function
	type(DTensor) function Dcompose_decompse(T1,Operar)
		...
		type(dimension)::dimen
		if(.not.T1%flag)then
			write(*,*)"There is no data in the Tensor,(Dcompose_decompse)"
			call error_stop()
		end if
		size1=size(Operar,1)
		dimen=T1%TenDim
		...
		call DstoreTenData(Dcompose_decompse,T1%DTensor_Data(1:T1%totalData))
...
!		other DElement of every row is input parameter of the function
	...
	subroutine DdimOperation2(T,Operat)
		integer,intent(in)::Operat(:)
		type(DTensor),intent(inout) :: T(:)
		integer::i
		do i=1,size(T)
			if(DgetFlag(T(i)))then
				call DdimOperation1(T(i),Operat)
			end if
		end do
		return
	end subroutine
	subroutine DdimOperation3(T,Operat)
		integer,intent(in)::Operat(:,:)
		type(DTensor),intent(inout) :: T(:)
		integer::i
		do i=1,size(T)
			if(DgetFlag(T(i)))then
				call DdimOperations(T(i),Operat)
			end if
		end do
		return
	end subroutine
...
!***************** equal_of_DTensor *****************
	logical function equal_of_DTensor(T1,T2)
		...
		l=count(abs(T1%DTensor_data(1:T1%totalData)-T2%DTensor_data(1:T2%totalData)).gt.1d-10)
		if(l.gt.0) then
			equal_of_DTensor=.false.
			return
		end if
...
!*****************  maxDElement  *****************
	real*8 function dmaxabsElement(T)
		type(DTensor),intent(in) :: T
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(dmaxabsElement)"
			call error_stop()
		end if
		dmaxabsElement=maxval(abs(T%DTensor_Data(1:T%totalData)))
		return
	end function
!*****************  maxRealDElement  *****************
	real*8 function dmaxElement(T)
		type(DTensor),intent(in) :: T
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(dmaxElement)"
			call error_stop()
		end if
		dmaxElement=maxval(T%DTensor_Data(1:T%totalData))
		return
	end function
!*****************  minDElement  *****************
	real*8 function dminabsElement(T)
		type(DTensor),intent(in) :: T
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(dminabsElement)"
			call error_stop()
		end if
		dminabsElement=minval(abs(T%DTensor_Data(1:T%totalData)))
		return
	end function
!*****************  minRealDElement  *****************
	real*8 function dminElement(T)
		type(DTensor),intent(in) :: T
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(dminElement)"
			call error_stop()
		end if
		dminElement=minval(T%DTensor_Data(1:T%totalData))
		return
	end function
!********************************************************************
!make QR decompostion: A = QR, Q is an orthomomal matrix, and R is an upper triangle
...
!X is a vector or a matrix,the dimension of which is the same as B
	type(DTensor) function Dlinequ(A,B)
		...
		Xdata=B
		call copyDTensor(Adata,A)!the subroutine ZGESV will change A
...
!			update at 2013.10.29
	real*8 function Dnorm2(Tr)
		...
		if(.not.Tr%flag)then
			write(*,*)"There is no data in the Tensor,(dnorm2)"
			call error_stop()
		end if
...
!		return  sqrt(<phi|phi>	)
	real*8 function Dnorm(Tr)
		type(DTensor),intent(in) :: Tr
		if(.not.Tr%flag)then
			write(*,*)"There is no data in the Tensor,(dnorm)"
			call error_stop()
		end if
...
!*****************  DElement   *****************
	subroutine DElement_subroutine2(i,j,Tdata,LD1,LD2,output)
	...
	subroutine DElement_subroutine3(i,j,k,Tdata,LD1,LD2,LD3,output)
	...
	subroutine DElement_subroutine4(i,j,k,l,Tdata,LD1,LD2,LD3,LD4,output)
	...
	real*8 function DElement(T,Tdim)
		...
		if(T%rank.eq.2)then
			...
			call DElement_subroutine2(Tdim(1),Tdim(2),T%DTensor_Data,T.dim.1,T.dim.2,DElement)
			return
		end if
		if(T%rank.eq.3)then
			...
			call DElement_subroutine3(Tdim(1),Tdim(2),Tdim(3),T%DTensor_Data,T.dim.1,T.dim.2,T.dim.3,DElement)
			return
		end if
		if(T%rank.eq.4)then
			...
			call dElement_subroutine4(Tdim(1),Tdim(2),Tdim(3),Tdim(4),T%DTensor_Data,T.dim.1,T.dim.2,T.dim.3,T.dim.4,DElement)
			return
		end if
		if(T%rank.eq.1)then
			...
			DElement=T%DTensor_Data(Tdim(1))
			return
		end if
...
!or [inde1_min,inde1_max,inde2_min,inde2_max] output data(inde1_min:inde1_max,inde2_min:inde2_max)
	...
	type(DTensor) function DsubTen(T,inde)	
		...
		if(size(inde).eq.4) then
			...
			call allocateDTensor(DsubTen,(/dim1,dim2/))
			call DsubTen_routine1(T%DTensor_Data,DsubTen%DTensor_data,T.dim.1,T.dim.2,dim1,&
		...
		if(size(inde).eq.2) then
			...
			select case (inde(1))
				case (-1)!output row
					call allocateDTensor(DsubTen,(/dim2/))
					call DsubTen_routine2(T%DTensor_Data,DsubTen%DTensor_Data,dim1,dim2,dim2,inde(2),.true.)
				case (-2)!
					call allocateDTensor(DsubTen,(/dim1/))
					call DsubTen_routine2(T%DTensor_Data,DsubTen%DTensor_Data,dim1,dim2,dim1,inde(2),.false.)
				case default 
		...
		select case (inde(1))
			case (-1)!output row
				...
				dim2=T.dim.2
				call allocateDTensor(DsubTen,(/dim1,dim2/))
				call DsubTen_routine3(T%DTensor_Data,DsubTen%DTensor_Data,T.dim.1,T.dim.2,dim1,&
	...
	subroutine DsubTen_routine1(Tensordata,outdata,TLD1,TLD2,OLD1,OLD2,istart,iend,jstart,jend)
		integer,intent(in)::TLD1,TLD2,OLD1,OLD2,istart,iend,jstart,jend
		real*8,intent(in)::Tensordata(TLD1,TLD2)
		real*8,intent(inout)::outdata(OLD1,OLD2)
		outdata=Tensordata(istart:iend,jstart:jend)
		return
	end subroutine
	subroutine DsubTen_routine2(Tensordata,outdata,TLD1,TLD2,OLD1,iindex,flag)
		integer,intent(in)::TLD1,TLD2,OLD1,iindex
		real*8,intent(in)::Tensordata(TLD1,TLD2)
		real*8,intent(inout)::outdata(OLD1)
	...
	subroutine DsubTen_routine3(Tensordata,outdata,TLD1,TLD2,OLD1,OLD2,istart,iend,flag)
...
!******************  Dmodify the data in the DTensor  **************
	subroutine DmodifyTen_array_subroutine1(i,Tdata,LenT,value,LenV)
		integer,intent(in)::i(2),LenT,LenV
		real*8,intent(inout)::Tdata(LenT)
		real*8,intent(in)::value(LenV)
		Tdata(i(1):i(2))=value
		return
	end subroutine
	subroutine DmodifyTen_val_subroutine2(i,j,Tdata,LD1,LD2,value)
		integer,intent(in)::i,j,LD1,LD2
		real*8,intent(inout)::Tdata(LD1,LD2)
		real*8,intent(in)::value
		Tdata(i,j)=value
		return
	end subroutine
	subroutine DmodifyTen_array_subroutine2(i,j,Tdata,LD1,LD2,value,LDV1,LDV2)
	...
	subroutine DmodifyTen_val_subroutine3(i,j,l,Tdata,LD1,LD2,LD3,value)
		integer,intent(in)::i,j,l,LD1,LD2,LD3
		real*8,intent(inout)::Tdata(LD1,LD2,LD3)
		real*8,intent(in)::value
		Tdata(i,j,l)=value
		return
	end subroutine
	subroutine DmodifyTen_array_subroutine3(i,j,l,Tdata,LD1,LD2,LD3,value,LDV1,LDV2,LDV3)
	...
	subroutine DmodifyTen_val_subroutine4(i,j,k,l,Tdata,LD1,LD2,LD3,LD4,value)
		integer,intent(in)::i,j,k,l,LD1,LD2,LD3,LD4
		real*8,intent(inout)::Tdata(LD1,LD2,LD3,LD4)
		real*8,intent(in)::value
		Tdata(i,j,k,l)=value
		return
	end subroutine
	subroutine DmodifyTen_array_subroutine4(i,j,k,l,Tdata,LD1,LD2,LD3,LD4,value,LDV1,LDV2,LDV3,LDV4)
...
! Dmodify the data in dimen of the DTensor
	subroutine DmodifyTen_val(Ten,dimen,val)
		...
		if(Ten%rank.eq.2) then
			...
			call DmodifyTen_val_subroutine2(dimen(1),dimen(2),Ten%DTensor_Data,Ten.dim.1,Ten.dim.2,val)
			return
		end if
		if(Ten%rank.eq.3) then
			...
			call DmodifyTen_val_subroutine3(dimen(1),dimen(2),dimen(3),Ten%DTensor_Data,&
		...
		if(Ten%rank.eq.4) then
			...
			call DmodifyTen_val_subroutine4(dimen(1),dimen(2),dimen(3),dimen(4),Ten%DTensor_Data,&
	...
	subroutine DmodifyTen_val1(Ten,ith,val)
		type(DTensor),intent(inout)::Ten
		integer,intent(in)::ith
		real*8,intent(in)::val
		Ten%DTensor_Data(ith)=val
		return
	end subroutine
	subroutine DmodifyTen_all(Ten,Allval)
		type(DTensor),intent(inout)::Ten
		real*8,intent(in)::Allval(*)
		call DCOPY(Ten%TotalData,Allval,1,Ten%DTensor_Data,1)
		return
	end subroutine
	subroutine DmodifyTen_array_val1(Ten,dimen1,val)
		type(DTensor),intent(inout)::Ten
		integer,intent(in)::dimen1(2)
		real*8,intent(in)::val(:)
		call DmodifyTen_array_subroutine1(dimen1,Ten%DTensor_Data,Ten%totalData,val,size(val))
		return
	end subroutine
	subroutine DmodifyTen_array_val2(Ten,dimen1,dimen2,val)
		type(DTensor),intent(inout)::Ten
		...
		call DmodifyTen_array_subroutine2(dimen1,dimen2,Ten%DTensor_Data,Ten.dim.1,Ten.dim.2,&
									val,size(val,1),size(val,2))
		return
	end subroutine
	subroutine DmodifyTen_array_val2_2(Ten,dimen1,dimen2,val)
		type(DTensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:)
		real*8,intent(in)::val(:)
		integer::row,col
	...
	subroutine DmodifyTen_array_val3(Ten,dimen1,dimen2,dimen3,val)
		type(DTensor),intent(inout)::Ten
		...
		call DmodifyTen_array_subroutine3(dimen1,dimen2,dimen3,Ten%DTensor_Data,Ten.dim.1,Ten.dim.2,&
									Ten.dim.3,val,size(val,1),size(val,2),size(val,3))
		return
	end subroutine
	subroutine DmodifyTen_array_val3_2(Ten,dimen1,dimen2,dimen3,val)
		type(DTensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:)
		real*8,intent(in)::val(:)
	...
	subroutine DmodifyTen_array_val4(Ten,dimen1,dimen2,dimen3,dimen4,val)
		type(DTensor),intent(inout)::Ten
		...
		call dmodifyTen_array_subroutine4(dimen1,dimen2,dimen3,dimen4,Ten%DTensor_Data,Ten.dim.1,Ten.dim.2,&
						Ten.dim.3,Ten.dim.4,val,size(val,1),size(val,2),size(val,3),size(val,4))
		return
	end subroutine
	subroutine DmodifyTen_array_val4_2(Ten,dimen1,dimen2,dimen3,dimen4,val)
		type(DTensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:),dimen4(:)
		real*8,intent(in)::val(:)
	...
	subroutine DmodifyTen_Tensor_all(Ten,dimen1,Ten2)
		type(DTensor),intent(inout)::Ten
		integer,intent(in)::dimen1(2)
		type(DTensor),intent(in)::Ten2
		integer::totaldata
		totaldata=dimen1(2)-dimen1(1)+1
		call Dcopy(totaldata,Ten2%DTensor_Data,1,Ten%DTensor_Data(dimen1(1):dimen1(2)),1)
		return
	end subroutine
	
	subroutine DmodifyTen_Tensor_val1(Ten,dimen1,Ten2,dimen21)
		type(DTensor),intent(inout)::Ten
		integer,intent(in)::dimen1(2),dimen21(2)
		type(DTensor),intent(in)::Ten2
		Ten%DTensor_Data(dimen1(1):dimen1(2))=Ten2%DTensor_Data(dimen21(1):dimen21(2))
		return
	end subroutine
	subroutine DmodifyTen_Tensor_val2(Ten,dimen1,dimen2,Ten2,dimen21,dimen22)
		type(DTensor),intent(inout)::Ten
		...
		call DmodifyTen_Tensor_val2_subroutine(dimen1,dimen2,Ten%DTensor_Data,Ten.dim.1,Ten.dim.2,&
						dimen21,dimen22,Ten2%DTensor_Data,Ten2.dim.1,Ten2.dim.2)
		return
	end subroutine
	subroutine DmodifyTen_Tensor_val2_2(Ten,dimen1,dimen2,Ten2)
		type(DTensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:)
		type(DTensor),intent(in)::Ten2
		if(Ten%rank.ne.2) then
		...
		call DmodifyTen_Tensor_val2_subroutine(dimen1,dimen2,Ten%DTensor_Data,Ten.dim.1,Ten.dim.2,&
						(/1,Ten2.dim.1/),(/1,Ten2.dim.2/),Ten2%DTensor_Data,Ten2.dim.1,Ten2.dim.2)
		return
	end subroutine
	subroutine DmodifyTen_Tensor_val3(Ten,dimen1,dimen2,dimen3,Ten2,dimen21,dimen22,dimen23)
		type(DTensor),intent(inout)::Ten
		...
		call DmodifyTen_Tensor_val3_subroutine(dimen1,dimen2,dimen3,Ten%DTensor_Data,Ten.dim.1,Ten.dim.2,&
						Ten.dim.3,dimen21,dimen22,dimen23,Ten2%DTensor_Data,Ten2.dim.1,Ten2.dim.2,Ten2.dim.3)
		return
	end subroutine
	subroutine DmodifyTen_Tensor_val3_2(Ten,dimen1,dimen2,dimen3,Ten2)
		type(DTensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:)
		type(DTensor),intent(in)::Ten2
	...
	subroutine DmodifyTen_Tensor_val4(Ten,dimen1,dimen2,dimen3,dimen4,Ten2,dimen21,dimen22,dimen23,dimen24)
		type(DTensor),intent(inout)::Ten
		...
		call DmodifyTen_Tensor_val4_subroutine(dimen1,dimen2,dimen3,dimen4,Ten%DTensor_Data,Ten.dim.1,Ten.dim.2,&
						Ten.dim.3,Ten.dim.4,dimen21,dimen22,dimen23,dimen24,Ten2%DTensor_Data,Ten2.dim.1,Ten2.dim.2,&
						Ten2.dim.3,Ten2.dim.4)
		return
	end subroutine
	subroutine DmodifyTen_Tensor_val4_2(Ten,dimen1,dimen2,dimen3,dimen4,Ten2)
		type(DTensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:),dimen4(:)
		type(DTensor),intent(in)::Ten2
	...
	subroutine DmodifyTen_Tensor_val2_subroutine(dim1,dim2,Data1,LD1,LD2,dim3,dim4,data2,LD3,LD4)
		integer,intent(in)::LD1,LD2,LD3,LD4,dim1(2),dim2(2),dim3(2),dim4(2)
		real*8,intent(inout)::Data1(LD1,LD2)
		real*8,intent(in)::Data2(LD3,LD4)
		Data1(dim1(1):dim1(2),dim2(1):dim2(2))=Data2(dim3(1):dim3(2),dim4(1):dim4(2))
		return
	end subroutine
	
	subroutine DmodifyTen_Tensor_val3_subroutine(dim1,dim2,dim3,Data1,LD1,LD2,LD3,dim4,dim5,dim6,data2,LD4,LD5,LD6)
		integer,intent(in)::LD1,LD2,LD3,LD4,LD5,LD6,dim1(2),dim2(2),dim3(2),dim4(2),dim5(2),dim6(2)
		real*8,intent(inout)::Data1(LD1,LD2,LD3)
		real*8,intent(in)::Data2(LD4,LD5,LD6)
		Data1(dim1(1):dim1(2),dim2(1):dim2(2),dim3(1):dim3(2))=Data2(dim4(1):dim4(2),dim5(1):dim5(2),dim6(1):dim6(2))
		return
	end subroutine
	subroutine DmodifyTen_Tensor_val4_subroutine(dim11,dim12,dim13,dim14,Data1,LD11,LD12,LD13,LD14,dim21,&
		...
		real*8,intent(inout)::Data1(LD11,LD12,LD13,LD14)
		real*8,intent(in)::Data2(LD21,LD22,LD23,LD24)
...
!			
	type(DTensor) function DSortTen(T,increase,inde)
		type(DTensor),intent(in)::T
		...
		call copyDTensor(DSortTen%DTensor_Data,T)
		if(present(inde)) then
			call sort(DSortTen%DTensor_Data,inde,increase)
		else
...
!**********************************************************************
	subroutine sent_DTensor(Ten1,Ten2,ID1,ID2,ierr,MPIcommon)
...
!**********************************************************************************	
		if(proID.eq.ID1) then
			call mpi_send(Ten1%DTensor_data,Ten1%totalData,MPI_double_precision,ID2,tag,mpi_comm,ierr)
		end if
		if(proID.eq.ID2) then
		...
		!	end if
			...
			call mpi_recv(Ten2%DTensor_data,Ten2%totalData,MPI_double_precision,ID1,tag,mpi_comm,istatus,ierr)
		end if
!**********************************************************************************	
		return
	end subroutine	
	
	subroutine BCAST_DTensor(Ten1,ID,ierr,MPIcommon)
...
!**********************************************************************************	
		...
		call MPI_BCAST(Ten1%DTensor_data,Ten1%totalData,MPI_double_precision,ID,mpi_comm,ierr)
!**********************************************************************************	
		return
	end subroutine
	
	subroutine sent_DTensorlink(link1,link2,ID1,ID2,ierr,MPIcommon)
...
!**********************************************************************************
	...
	subroutine BCAST_DTensorlink(link1,ID,ierr,MPIcommon)
