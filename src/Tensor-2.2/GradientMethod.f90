...
!		real*8 a=Gradient_Method(inoutP,max_flag,lenofinP,func,GradientFunction) 
!		real*8 a=Gradient_Method(inoutP,para,max_flag,lenofinP,func,GradientFunction) 
...
!			if present GradientFunction, the gradient will be GradientFunction(Ten) or searchFunction(Ten,para)
!			  if not The gradient is  (searchFunction(P+Delta_P)-searchFunction(P-Delta_P))/ (2*Delta_P)
!
!
...
!Search inoutP
	...
	real*8 function Pstep(P,max_flag,GMsearchFunction,GradientFunction)
		type(DTensor),intent(inout)::P
		logical,intent(in)::max_flag
		real*8,external::GMsearchFunction
		Type(DTensor),optional,external::GradientFunction
		real*8::f0,f1,step
		real*8::Tendata(3)
		...
		if(present(GradientFunction)) then
			Sdir=GradientFunction(P0)
			Sdir=Sdir/dnorm(Sdir)
			if(.not.max_flag) Sdir= (-1d0)*Sdir
		else
...
!the search function is GMsearchFunction(inoutP,point_parameter)
!Search inoutP
	real*8 function Pstep_para(P,point_parameter,max_flag,GMsearchFunction,GradientFunction)result(Pstep)
		type(DTensor),intent(inout)::P
		type(DTensor),intent(in)::point_parameter
		logical,intent(in)::max_flag
		real*8,external::GMsearchFunction
		...
		step=max_step
		if(present(GradientFunction)) then
			Sdir=GradientFunction(P0,point_parameter)
			Sdir=Sdir/dnorm(Sdir)
			if(.not.max_flag) Sdir= (-1d0)*Sdir
		else
...
! 
	real*8 function Gradient_Method1(inoutP,max_flag,lenofinP,GMsearchFunction,GradientFunction)
		...
		newf=Pstep(P,max_flag,GMsearchFunction,GradientFunction)
		i=1
	...
	real*8 function Gradient_Method2(inoutP,max_flag,GMsearchFunction,GradientFunction)
		type(DTensor),intent(inout)::inoutP
		logical,intent(in)::max_flag
		real*8,external::GMsearchFunction
		Type(DTensor),optional,external::GradientFunction
...
!the search function is GMsearchFunction(inoutP,point_parameter)
!Search inoutP
	real*8 function Gradient_Method_para(inoutP,point_parameter,max_flag,lenofinP,GMsearchFunction,GradientFunction)
		type(DTensor),intent(inout)::inoutP
		type(DTensor),intent(in)::point_parameter
		logical,intent(in)::max_flag
		integer,intent(in)::lenofinP
		...
		newf=Pstep_para(P,point_parameter,max_flag,GMsearchFunction,GradientFunction)
		i=1
		do while(dabs(newf-f0).gt.stop_error)
			f0=newf
			newf=Pstep_para(P,point_parameter,max_flag,GMsearchFunction,GradientFunction)
	...
	real*8 function Gradient_Method2_para(inoutP,point_parameter,max_flag,GMsearchFunction,GradientFunction)
		type(DTensor),intent(inout)::inoutP
		type(DTensor),intent(in)::point_parameter
		logical,intent(in)::max_flag
		real*8,external::GMsearchFunction
