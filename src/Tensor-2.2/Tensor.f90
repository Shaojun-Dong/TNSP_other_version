...
!
!		modify on 2014.12
!			1. Rewrite (=) T1=T2, T1 will not deallocate if size(T1%Tensor_Data)>=T2%TotalData
!		It will save time on not doing the deallocate.In the code, one can call allocateTensor(T)
!		To create a big Tensor,and than work on it. The array of T%Tensor_Data will not deallocate
!		untill it cannot store the data.So T%TotalData is the length of Data in T,not 
!		size(T%Tensor_Data).
!			2. array=T, will not allocate the array. But CopyTensor(array,T) will allocate array
!
...
!*********		define data type	***************
	type  Tensor
		...
		logical,private :: flag=.false. !if flag=true ,it means there are data in Tensor
	end type Tensor
...
!*******************************************
!~*****	 use in SVD **********
		complex*16,save,private,allocatable :: SVD_T1data(:,:)
		complex*16,save,private,allocatable :: SVD_T2data(:,:)
		complex*16,save,private,allocatable :: SVD_Tdata(:)
		complex*16,save,private,allocatable :: SVD_work(:)
		integer,save,private,allocatable :: SVD_iw(:)
		real*8,save,private,allocatable :: SVD_rw(:),SVD_sdata(:)
		logical,save,private::not_SVD_flag=.true.
		integer,save,private::SVD_rwdim,SVD_min_MN,SVD_M,SVD_N,SVD_lw,SVD_totalData
!*******************************************
	interface TMprint
...
!one can use subroutine setTensorName(T,Tensorname,nameID) to set a name to the Tensor
	...
	interface generate
		module procedure generate_NoName!generate a random Tensor 
		module procedure generate_dim
	...
	interface modify
		...
		module procedure modifyTen_all!Ten%Tensor_Data(1:Ten%TotalData)=Allval,call modify(T,Allval)
		module procedure modifyTen_val!T(i)=value or T(i,j)=value, if the rank<=4 ,the speed is the same as array,The program will check the input
		                              !call modify(T,dimen,val),dimen=(/ith/) or dimen=(/ith,ith/), dimen=(/ith,jth,kth,../)
		module procedure modifyTen_val1!call modify(T,ith,val)
		module procedure modifyTen_array_val1!T(i1:i2)=value(:),the speed is the same as array
		                                     !call modify(T,dimen,array),dimen=(/i1,i2/)
		module procedure modifyTen_array_val2!T(i1:i2,j1:j2)=value(:,:),the speed is the same as array
		                                     !call modify(T,dimen1,dimen2,array),dimen1=(/i1,i2/),dimen3=(/j1,j2/)    
		module procedure modifyTen_array_val2_2!T(i1:i2,j1:j2)=value(:),the speed is the same as array        
		module procedure modifyTen_array_val3!T(i1:i2,j1:j2,l1:l2)=value(:,:,:),the speed is the same as array
		                         !call modify(T,dimen1,dimen2,dimen3,value),dimen1=(/i1,i2/),dimen1=(/j1,j2/),dimen3=(/l1,l2/)  
		module procedure modifyTen_array_val3_2!T(i1:i2,j1:j2,l1:l2)=value(:),the speed is the same as array
		                   !call modify(T,dimen1,dimen2,dimen3,value),dimen1=(/i1,i2/),dimen1=(/j1,j2/),dimen3=(/l1,l2/) 
		module procedure modifyTen_array_val4!T(i1:i2,j1:j2,k1:k2,l1:l2)=value(:,:,:,:),the speed is the same as array
		                         !call modify(T,dimen1,dimen2,dimen3,value),dimen1=(/i1,i2/),dimen1=(/j1,j2/),dimen3=(/l1,l2/)  
		module procedure modifyTen_array_val4_2!T(i1:i2,j1:j2,k1:k2,l1:l2)=value(:),the speed is the same as array
		module procedure modifyTen_Tensor_val1!T(i1:i2)=T2(ii1:ii2),the speed is the same as array,T2 is a Tensor
		                   !call modify(T,dimen1,T2,dimen21),dimen1=(/i1,i2/),dimen21=(/ii1,ii2/)
		module procedure modifyTen_Tensor_val2!T(i1:i2,j1:j2)=T2(ii1:ii2,jj1:jj2),the speed is the same as array,T2 is a Tensor
		  !call modify(T,dimen1,dimen2,T,dimen21,dimen22),dimen1=(/i1,i2/),dimen1=(/j1,j2/),dimen21=(/ii1,ii2/) ,dimen22=(/jj1,jj2/) 
		module procedure modifyTen_Tensor_val2_2!T(i1:i2,j1:j2)=T2,the speed is the same as array,T2 is a Tensor
			!call modify(T,dimen1,dimen2,T),dimen1=(/i1,i2/),dimen1=(/j1,j2/)
		module procedure modifyTen_Tensor_val3!T(i1:i2,j1:j2,l1:l2)=T2(:,:,:),the speed is the same as array,T2 is a Tensor
		module procedure modifyTen_Tensor_val3_2!T(i1:i2,j1:j2,l1:l2)=T2,the speed is the same as array,T2 is a Tensor
		module procedure modifyTen_Tensor_val4!T(i1:i2,j1:j2,k1:k2,l1:l2)=T2(:,:,:,:),the speed is the same as array,T2 is a Tensor
		module procedure modifyTen_Tensor_val4_2!T(i1:i2,j1:j2,k1:k2,l1:l2)=T2,the speed is the same as array,T2 is a Tensor
		module procedure modifyTen_Tensor_all!Ten%Tensor_Data(i1:i2)=T2%Tensor_Data
	end interface
!************************************************************	
...
!	 Tensor_Data of Tensor.
	interface dimOperation
		...
		module procedure dimOperation2!use on Tensor array
		module procedure dimOperation3!use on Tensor array
	end interface
...
!use Tensor%Totaldata to judge if can modify the dimension
	interface resetdim
...
! 	input Tensor should be in its original dimenison,there is no contract on it
	interface Tenproduct
		...
		module procedure Tenproduct_noName2
		module procedure Tenproduct_Name!In put dimension as character
		module procedure Tenproduct_Name2
		module procedure Tenproduct_Intname
		module procedure Tenproduct_Intname2
		module procedure Tenproduct_name_int
		module procedure Tenproduct_name_int2
		module procedure Tenproduct_intname_int
		module procedure Tenproduct_intname_int2
		module procedure Tenproduct_int_name
		module procedure Tenproduct_int_name2
		module procedure Tenproduct_int_intname
		module procedure Tenproduct_int_intname2
...
!		if inde larger than rank ,the function will return no change	
	...
	interface operator(-)
		...
		module procedure minus_DTen!Tensor - DTensor
		module procedure minus_DTen2!DTensor - Tensor
...
!		the function productTensor add at 2013.10.28
	...
	interface operator(.numx.)
		module procedure TensorProduct1Dim!the same as dot product but do not conjugating the first vector,The Tensor will be regard as a vector
	end interface
...
!******************************************************************************************************
	...
	interface operator(.i.)
		...
		module procedure Element!output the data in the Tensor of Tim=(i,j),if rank<=4, the speed is the same as array,The program will check the input
...
!allocate Tensor according to the dimen		
	...
	interface copyTensor
		module procedure copyTensortoTensor!Do not allocate the Tensor
		module procedure copy_dim1!Vec=T ,Vec is a vector and T are Tensor,vec=T%Tensor_Data,used for any case,Mat will be allocate
		module procedure copy_dim2!Mat=T ,Mat is a matrix and T are Tensor,Mat=T%Tensor_Data,used only rank=2,Mat will be allocate
		module procedure copy_dim3!Mat=T ,Mat is a rank=3 and T are Tensor,Mat=T%Tensor_Data,used only rank=3,Mat will be allocate
	end interface
	interface assignment(=)
		...
		module procedure copyTensordim1!Vec=T ,Vec is a vector and T are Tensor,vec=T%Tensor_Data,used for any case,Vec should be allocated
		module procedure copyTensordim2!Mat=T ,Mat is a matrix and T are Tensor,Mat=T%Tensor_Data,used only rank=2,Mat should be allocated
		module procedure copyTensordim3!Mat=T ,Mat is a rank=3 and T are Tensor,Mat=T%Tensor_Data,used only rank=3,Mat should be allocated
		...
		module procedure assignmentTenNum!T=dcmplx(a,b),All the element of T will be dcmplx(a,b),if no data,T=(/dcmplx(a,b)/)
		module procedure assignmentTenNumReal!T=a,All the element of T will be a,if no data,T=(/a/)
...
!
!		TenNameorder:output the order in the dimension
!
...
!
!		emptyTensor: set Tensor to a emptyTensor, but do not deallocate memery
!
	...
	subroutine assignmentTen(T,T2)
		...
		if(T2%flag) then
			...
			if(allocated(T%Tensor_Data)) then
				if(size(T%Tensor_Data).lt.length) then!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!If old data longer than new,do not allocate
...
!The output Tensor should be allocated
	subroutine copyTensortoTensor(T,T2)
		type(Tensor),intent(inout) ::T
		type(Tensor),intent(in) :: T2
		integer::length
		...
		T%totalData=T2%totalData
		T%flag=T2%flag
		call ZCOPY(T%totalData,T2%Tensor_Data,1,T%Tensor_Data,1)
		return
	end subroutine
!Do not allocate the Tensor data
...
!allocate Tensor according to the dimen
	...
	!	end if
		...
		if(T%totalData.le.0) then
			write(*,*)"ERROR in allocateTensor"
			call error_stop()
		end if
		call allocateCheck(T%Tensor_Data,T%totalData)
	...
	subroutine Ten_DTen(T,DT)
		...
		if(Dgetflag(DT)) then
			!T=DT%DTensor_Data(1:DgetTotalData(DT))
			call storeTenData_dim1(T,dcmplx(DT%DTensor_Data(1:DgetTotalData(DT)),0d0))
			T%rank=dgetRank(DT)
			T%TenDim=.subdim.DT
	...
	subroutine assignmentTenNum(T,num)
		type(Tensor),intent(inout)::T
	...
	subroutine assignmentTenNumreal(T,num)
		type(Tensor),intent(inout)::T
...
!***********	storeTenData  **************
	subroutine storeTenData_dim1(T,inData)
		...
		length=size(inData)
		if(allocated(T%Tensor_data))then
			if(length.gt.size(T%Tensor_data)) then!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!If old data longer than new,do not allocate
				deallocate(T%Tensor_Data)
				allocate(T%Tensor_Data(length))
			end if
		else
...
!	build a Tensor
	...
	type(Tensor) function buildTen8(TenDim,Tensor_data)
		...
		complex*16,intent(in)::Tensor_data(:)
		call copydimension(Dimen,TenDim)
...
!		generate a Tensor with random number
	type(Tensor) function generate_noName(Tdim,region) result (T)
		...
		do i=1,totalData
			...
			Tensor_data(i)=(Tensor_data(i)*delmum)+dcmplx(minnum,minnum)
		end do
		TenDim=Tdim
	...
	type(Tensor) function generate_dim(Tdim,region) result (T)
		type(Dimension),intent(in) :: Tdim
		real*8,optional,intent(in)::region(2)
		integer :: i
		real*8 ::minnum,maxnum,delmum
		if(present(region))then
			minnum=region(1)
			maxnum=region(2)
		else
		...
		delmum=maxnum-minnum
		call allocateTensor(T,Tdim)
		do i=1,T%totalData
			T%Tensor_data(i)=dcmplx(randomnumber(),randomnumber())
			T%Tensor_data(i)=(T%Tensor_data(i)*delmum)+dcmplx(minnum,minnum)
		end do
		return
	end function
!*********************  generate *********************
...
!*********************** TensorName   **********************
	...
	subroutine setTensorName9(T,oldTensorName,oldDimenName,newTensorName,newDimenName)
		...
		call setDimName(T%TenDim,oldTensorName,oldDimenName,newTensorName,newDimenName)
		return
	end subroutine
	...
	function outTensorIntName(T,ith)
		...
		outTensorIntName=outIntNameTen(T%TenDim,ith)
		return
	end function
	function outDimensionIntName(T,ith)
		...
		outDimensionIntName=outIntNameDim(T%TenDim,ith)
		return
	end function
	...
	function outAllTensorIntName(T)
		...
		do i=1,T%rank
			outAllTensorIntName(i,:)=outIntNameTen(T%TenDim,i)
		end do
		return
	end function
	function outAllDimensionintName(T)
		...
		do i=1,T%rank
			outAllDimensionintName(i,:)=outIntNameDim(T%TenDim,i)
		end do
		return
	end function
	integer function TenNameorder1(T,w)
		...
		character(len=*),intent(in)::w
		integer :: inde
		TenNameorder1=Nameorder(T%TenDim,w)
		return
	end function
	function TenNameorder2(T,w)
		...
		TenNameorder2=Nameorder(T%TenDim,w)
		return
	end function
	integer function TenNameorder3(T,TensorName,DimenName)
		...
		integer :: inde
		TenNameorder3=Nameorder(T%TenDim,TensorName,DimenName)
		return
	end function
	function TenNameorder4(T,TensorName,DimenName)
		...
		TenNameorder4=Nameorder(T%TenDim,TensorName,DimenName)
		return
	end function
...
!		generate a Tensor with all element is 0d0
	type(Tensor) function zeroTen1(Tdim) result (T)
		integer,intent(in) :: Tdim(:)
		integer :: rank,totalData,i
		rank=size(Tdim)
		totalData=product(Tdim)
...
!		generate a Tensor with all element is 0d0
	type(Tensor) function zeroTen2(Tdim) result (T)
		type(Dimension),intent(in) :: Tdim
		call allocateTensor(T,Tdim)
		T%Tensor_data=dcmplx(0d0,0d0)
		return
...
!********************* print_Tensor *********************       
	subroutine Tprint1(T,words)
		...
		if(T%flag) then
			...
			write(*,*) T%Tensor_data(1:T%totalData)
			call Dprint(T%TenDim)
			write(*,*) "***end***"
			write(*,*) ""
		else
...
!********************* print_dim *********************            
	...
	subroutine Tprint2(T,printtype,words)
		type(Tensor),intent(in) :: T
		...
		if(T%flag) then
			...
			case (1)
				write(*,*) real(T%Tensor_data(1:T%totalData))
			case (2)
				write(*,*) aimag(T%Tensor_data(1:T%totalData))
			case (3)
				write(*,'(99999F12.8)') real(T%Tensor_data(1:T%totalData))
			case (4)
				write(*,'(99999F12.8)') aimag(T%Tensor_data(1:T%totalData))
			case (5)
				write(*,'(99999F10.2)') real(T%Tensor_data(1:T%totalData))
			case (6)
				write(*,'(99999F10.2)') aimag(T%Tensor_data(1:T%totalData))
			end 	select
			call Dprint(T%TenDim)
			write(*,*) "***end***"
			write(*,*) ""
	...
	subroutine Tprint2_file(T,printtype,fileaddress,fileunit,replace)
		...
		if(T%flag) then
			...
			select case (printtype)
			case (1)
				write(fileunit,*) real(T%Tensor_data(1:T%totalData))
			case (2)
				write(fileunit,*) aimag(T%Tensor_data(1:T%totalData))
			case (3)
				write(fileunit,'(99999F12.8)') real(T%Tensor_data(1:T%totalData))
			case (4)
				write(fileunit,'(99999F12.8)') aimag(T%Tensor_data(1:T%totalData))
			case (5)
				write(fileunit,'(99999F10.2)') real(T%Tensor_data(1:T%totalData))
			case (6)
				write(fileunit,'(99999F10.2)') aimag(T%Tensor_data(1:T%totalData))
			end 	select
		!	call Dprint(T%TenDim)
			write(fileunit,*) "***end***"
...
!********************* print_Matrix *********************
	...
	subroutine TMprint2(T,printtype,words)
		type(Tensor),intent(in) :: T
		integer,intent(in)::printtype
		complex*16,allocatable :: Tdata(:,:)
		complex*16,allocatable :: Tdata3(:,:,:)
...
!cccccccccccccccc add          cccccccccccccccccc
	type(Tensor) function add(T1,T2)
		...
		call zaxpy (add%Totaldata, dcmplx(1d0,0d0), T2%Tensor_Data,1,add%Tensor_Data,1)
		return
	end function
	type(Tensor) function add_DTen1(T1,T2)
		...
		if((.not.T1%flag).or.(.not.DgetFlag(T2)))then
			write(*,*)"There is no data in the Tensor,(+)"
			call error_stop()
		end if
		dim1=T1%TenDim
		...
		add_DTen1%Totaldata=T1%totaldata
		add_DTen1%flag=T1%flag
		add_DTen1%TenDim=T1%TenDim
		add_DTen1%rank=T1%rank
		allocate(add_DTen1%Tensor_Data(add_DTen1%Totaldata))
		call ZCOPY(add_DTen1%Totaldata,T1%Tensor_Data,1,add_DTen1%Tensor_Data,1)
		call zaxpy (add_DTen1%Totaldata, dcmplx(1d0,0d0), dcmplx(T2%DTensor_Data,0d0),1,add_DTen1%Tensor_Data,1)
	!	realData=T2
	...
	type(Tensor) function add_DTen2(T1,T2)
		...
		type(Dimension)::dim1,dim2
		real*8,allocatable::realData(:)
		if((.not.T2%flag).or.(.not.DgetFlag(T1)))then
			write(*,*)"There is no data in the Tensor,(+)"
			call error_stop()
		end if
		dim1=.subDim.T1
		...
		add_DTen2%Totaldata=DgetTotaldata(T1)
		add_DTen2%flag=DgetFlag(T1)
		add_DTen2%TenDim=dim1
		add_DTen2%rank=DgetRank(T1)
		allocate(add_DTen2%Tensor_Data(add_DTen2%Totaldata))
		call ZCOPY(add_DTen2%Totaldata,dcmplx(T1%DTensor_Data,0),1,add_DTen2%Tensor_Data,1)
		call zaxpy (add_DTen2%Totaldata, dcmplx(1d0,0d0), T2%Tensor_Data,1,add_DTen2%Tensor_Data,1)
		!realData=T1
	...
	type(Tensor) function add_num(T1,num)
		...
		call copydimension(dimT,T1%TenDim)
		T2=eye(dimT)
		add_num=add(T1,T2)
		return
	end function
...
!cccccccccccccccc    minus       cccccccccccccccccc		
	type(Tensor) function minus(T1,T2)
		...
		minus%rank=T1%rank
		allocate(minus%Tensor_Data(minus%Totaldata))
		call ZCOPY(minus%Totaldata,T1%Tensor_Data,1,minus%Tensor_Data,1)
		call zaxpy (minus%Totaldata,dcmplx(-1d0,0d0), T2%Tensor_Data,1,minus%Tensor_Data,1)
		return
	end function
	type(Tensor) function minus_DTen(T1,T2)
		type(Tensor),intent(in) :: T1
		type(DTensor),intent(in) ::T2
		type(Dimension)::dim1,dim2
		if((.not.T1%flag).or.(.not.Dgetflag(T2)))then
		...
		minus_DTen%Totaldata=T1%totaldata
		minus_DTen%flag=T1%flag
		minus_DTen%TenDim=T1%TenDim
		minus_DTen%rank=T1%rank
		allocate(minus_DTen%Tensor_Data(minus_DTen%Totaldata))
		call ZCOPY(minus_DTen%Totaldata,T1%Tensor_Data,1,minus_DTen%Tensor_Data,1)
		call zaxpy (minus_DTen%Totaldata,dcmplx(-1d0,0d0),dcmplx(T2%DTensor_Data,0d0),1,minus_DTen%Tensor_Data,1)
		return
	end function
	type(Tensor) function minus_DTen2(T1,T2)
		type(DTensor),intent(in) :: T1
		type(Tensor),intent(in) ::T2
		type(Dimension)::dim1,dim2
		if((.not.Dgetflag(T1)).or.(.not.getflag(T2)))then
			write(*,*)"There is no data in the Tensor,(-)"
			call error_stop()
		end if
		dim1=.subDim.T1
		...
		minus_DTen2%Totaldata=DgetTotaldata(T1)
		minus_DTen2%flag=DgetFlag(T1)
		minus_DTen2%TenDim=dim1
		minus_DTen2%rank=DgetRank(T1)
		allocate(minus_DTen2%Tensor_Data(minus_DTen2%Totaldata))
		call ZCOPY(minus_DTen2%Totaldata,dcmplx(T1%DTensor_Data,0d0),1,minus_DTen2%Tensor_Data,1)
		call zaxpy (minus_DTen2%Totaldata,dcmplx(-1d0,0d0),T2%Tensor_Data,1,minus_DTen2%Tensor_Data,1)
		return
	end function
...
!cccccccccccccccc divide          cccccccccccccccccc	
	type(Tensor) function divide(T1,T2)
		...
		divide=set(dim1,T1%Tensor_data(1:T1%Totaldata)/T2%Tensor_data(1:T2%Totaldata))
		return
	end function
!cccccccccccccccc divide_number          cccccccccccccccccc	
	type(Tensor) function divide_number(T1,num)
		...
		allocate(divide_number%Tensor_Data(divide_number%Totaldata))
		call ZCOPY(divide_number%Totaldata,T1%Tensor_Data,1,divide_number%Tensor_Data,1)
		call ZSCAL(divide_number%Totaldata,1d0/num,divide_number%Tensor_Data,1)
...
!ccccccccccccccccc divide_number          cccccccccccccccccc	
	type(Tensor) function divide_numberReal(T1,num)
		...
		if(num.lt.1d-16)then
			write(*,*)"The num is too small in (/): Tensor/0."
			write(*,*)"Stop"
			call error_stop()
		end if
		...
		allocate(divide_numberReal%Tensor_Data(divide_numberReal%Totaldata))
		call ZCOPY(divide_numberReal%Totaldata,T1%Tensor_Data,1,divide_numberReal%Tensor_Data,1)
		call ZSCAL(divide_numberReal%Totaldata,dcmplx(1d0/num,0d0),divide_numberReal%Tensor_Data,1)
...
!ccccccccccccccccc divide_number          cccccccccccccccccc	
	type(Tensor) function divide_numberReal4(T1,num)
		...
		allocate(divide_numberReal4%Tensor_Data(divide_numberReal4%Totaldata))
		call ZCOPY(divide_numberReal4%Totaldata,T1%Tensor_Data,1,divide_numberReal4%Tensor_Data,1)
		call ZSCAL(divide_numberReal4%Totaldata,dcmplx(1d0/num,0d0),divide_numberReal4%Tensor_Data,1)
...
!cccccccccccccccc multiply_Tensor          cccccccccccccccccc	
	type(Tensor) function multiply_Tensor(T1,T2)
		...
		type(Dimension)::dim1,dim2
		if((.not.T1%flag).or.(.not.T2%flag))then
			write(*,*)"There is no data in the Tensor,(.elex.)"
			call error_stop()
		end if
		dim1=T1%TenDim
		dim2=T2%TenDim
		...
		multiply_Tensor=set(dim1,T1%Tensor_data(1:T1%Totaldata)*T2%Tensor_data(1:T2%Totaldata))
		return
	end function
!cccccccccccccccc multiply_number          cccccccccccccccccc	
	type(Tensor) function multiply_number(T1,num)
		...
		allocate(multiply_number%Tensor_Data(multiply_number%Totaldata))
		call ZCOPY(multiply_number%Totaldata,T1%Tensor_Data,1,multiply_number%Tensor_Data,1)
		call ZSCAL(multiply_number%Totaldata,num,multiply_number%Tensor_Data,1)
		...
		allocate(multiply_number_%Tensor_Data(multiply_number_%Totaldata))
		call ZCOPY(multiply_number_%Totaldata,T1%Tensor_Data,1,multiply_number_%Tensor_Data,1)
		call ZSCAL(multiply_number_%Totaldata,num,multiply_number_%Tensor_Data,1)
...
!cccccccccccccccc multiply_real          cccccccccccccccccc	
	type(Tensor) function multiply_real(T1,num)
		...
		allocate(multiply_real%Tensor_Data(multiply_real%Totaldata))
		call ZCOPY(multiply_real%Totaldata,T1%Tensor_Data,1,multiply_real%Tensor_Data,1)
		call ZSCAL(multiply_real%Totaldata,dcmplx(num,0d0),multiply_real%Tensor_Data,1)
	...
	type(Tensor) function multiply_real_(num,T1)
		...
		allocate(multiply_real_%Tensor_Data(multiply_real_%Totaldata))
		call ZCOPY(multiply_real_%Totaldata,T1%Tensor_Data,1,multiply_real_%Tensor_Data,1)
		call ZSCAL(multiply_real_%Totaldata,dcmplx(num,0d0),multiply_real_%Tensor_Data,1)
...
!ccccccccccccccccc multiply_real4          cccccccccccccccccc	
	type(Tensor) function multiply_real4(T1,num)
		...
		allocate(multiply_real4%Tensor_Data(multiply_real4%Totaldata))
		call ZCOPY(multiply_real4%Totaldata,T1%Tensor_Data,1,multiply_real4%Tensor_Data,1)
		call ZSCAL(multiply_real4%Totaldata,dcmplx(num,0d0),multiply_real4%Tensor_Data,1)
...
!  lenD is used in zcpoy, here lenD=LD1
	subroutine permutation_rank3_data1(outdata,indata,LD1,LD2,LD3,lenD)
		integer,intent(in)::LD1,LD2,LD3,lenD
		complex*16::outdata(LD1,LD3,LD2)
		complex*16::indata(LD1,LD2,LD3)
		integer::i,j,k
		!do i=1,LD1
...
!  lenD is used in zcpoy, here lenD=LD1*LD3
	subroutine permutation_rank3_data2(outdata,indata,LD1,LD2,LD3,lenD)
		integer,intent(in)::LD1,LD2,LD3,lenD
		complex*16::outdata(LD3,LD2,LD1)
		complex*16::indata(LD1,LD2,LD3)
		integer::i
		do i=1,LD2
			outdata(:,i,:)=transpose(indata(:,i,:))
...
!  lenD is used in zcpoy, here lenD=LD1*LD2
	subroutine permutation_rank3_data3(outdata,indata,LD1,LD2,LD3,lenD)
		integer,intent(in)::LD1,LD2,LD3,lenD
		complex*16::outdata(LD2,LD1,LD3)
		complex*16::indata(LD1,LD2,LD3)
		integer::i
		do i=1,LD3
			outdata(:,:,i)=transpose(indata(:,:,i))
...
!  lenD is used in zcpoy, here lenD=LD1*LD2
	subroutine permutation_rank2_data(outdata,indata,LD1,LD2,lenD)
		integer,intent(in)::LD1,LD2,lenD
		complex*16::outdata(LD2,LD1)
		complex*16::indata(LD1,LD2)
...
!ccccccccccccccccc permute_rank3   cccccccccccccccccc		
	type(Tensor) function permute_rank3(T1,index_not_permute)result(Res)
		...
		if(index_not_permute.eq.1) then
			call allocateTensor(Res, Dimpermute(T1%TenDim,(/1,3,2/)) )
			lenD=dimen(1)
			call permutation_rank3_data1(Res%Tensor_Data,T1%Tensor_Data,dimen(1),dimen(2),dimen(3),lenD)
		end if
		if(index_not_permute.eq.2) then
			call allocateTensor(Res, Dimpermute(T1%TenDim,(/3,2,1/)) )
			lenD=dimen(1)*dimen(3)
			call permutation_rank3_data2(Res%Tensor_Data,T1%Tensor_Data,dimen(1),dimen(2),dimen(3),lenD)
		end if
		if(index_not_permute.eq.3) then
			call allocateTensor(Res, Dimpermute(T1%TenDim,(/2,1,3/)) )
			lenD=dimen(1)*dimen(2)
			call permutation_rank3_data3(Res%Tensor_Data,T1%Tensor_Data,dimen(1),dimen(2),dimen(3),lenD)
		end if
	 end function
...
!cccccccccccccccc permute_rank2   cccccccccccccccccc			 
	type(Tensor) function permute_rank2 (T)result(Res)
		...
		dimen=T%TenDim
		call allocateTensor(Res, Dimpermute(T%TenDim,(/2,1/)) )
		call permutation_rank2_data(Res%Tensor_Data,T%Tensor_Data,dimen(1),dimen(2),product(dimen))
	end function
!cccccccccccccccc permutation   cccccccccccccccccc
	...
	subroutine permutation_data3(T1data,index_not_permute,dimens,totaldata)
		integer,intent(in) ::   index_not_permute,totaldata
		complex*16,intent(inout) :: T1data(totaldata)
		type(Dimension),intent(inout) ::dimens
		complex*16::oldData(totaldata)
		integer::dimen(3),lenD
		dimen=dimens
		call zcopy(totaldata,T1data,1,oldData,1)
		if(index_not_permute.eq.1) then
			lenD=dimen(1)
			call permutation_rank3_data1(T1data,oldData,dimen(1),dimen(2),dimen(3),lenD)
			dimens=Dimpermute(dimens,(/1,3,2/))
			return
		end if
		if(index_not_permute.eq.2) then
			lenD=dimen(1)*dimen(3)
			call permutation_rank3_data2(T1data,oldData,dimen(1),dimen(2),dimen(3),lenD)
			dimens=Dimpermute(dimens,(/3,2,1/))
			return
		end if
		if(index_not_permute.eq.3) then
			lenD=dimen(1)*dimen(2)
			call permutation_rank3_data3(T1data,oldData,dimen(1),dimen(2),dimen(3),lenD)
	...
	subroutine permutation_data2 (T1data,dimens,totaldata)
		...
		dimen=dimens
		call zcopy(totaldata,T1data,1,oldData,1)
		call permutation_rank2_data(T1data,oldData,dimen(1),dimen(2),product(dimen))
		dimens=Dimpermute(dimens,(/2,1/))
	end subroutine
	subroutine permutefo_data(Tdata,inde,dimen,totaldata)
		complex*16,intent(inout)::Tdata(:)
...
!
	...
	type(Tensor) function permutefo_name(T,indechar)result(permutefo)
		...
		if(inde.le.0) then
			write(*,*)"ERROR in function permutefo_name,Can not find the name"
			write(*,*)"stop"
			call error_stop()
		end if
		if(inde.eq.1) then
...
!
	...
	type(Tensor) function permuteback_name(T,indechar)result(permuteback)
		...
		integer::oper(2,3)
		if(.not.T%flag)then
			write(*,*)"There is no data in the Tensor,(permuteback_name)"
			call error_stop()
		end if
		rank=getRank(T)
		inde=Nameorder(T%TenDim,indechar)
		if(inde.gt.rank) then
			write(*,*)"ERROR in function permuteback_name"
			write(*,*)"stop"
			call error_stop()
		end if
		if(inde.le.0) then
			write(*,*)"ERROR in function permuteback_name,Can not find the name"
			write(*,*)"stop"
			call error_stop()
		end if
		if(inde.eq.rank) then
...
type(Tensor) function Tenproduct_noName(T1_,i1,T2_,i2) result(T)
	...
	type(Tensor) function Tenproduct_noName2(T1_,i1,T2_,i2) result(T)
		type(Tensor),intent(in) :: T1_,T2_
		integer,intent(in) :: i1,i2
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
			write(*,*)"ERROR in Tenproduct"
	...
	type(Tensor) function Tenproduct_name2(T1_,name1,T2_,name2) result(T)
		type(Tensor),intent(in) :: T1_,T2_
		character(len=*),intent(in)::name1,name2
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
			write(*,*)"ERROR in Tenproduct"
	...
	type(Tensor) function Tenproduct_Intname(T1_,name1,Nimname1,T2_,name2,Nimname2) result(T)
		type(Tensor),intent(in) :: T1_,T2_
		...
		i1=Nameorder(T1_%TenDim,name1,Nimname1)
		i2=Nameorder(T2_%TenDim,name2,Nimname2)
		rank1=T1_%rank
		rank2=T2_%rank
		leni1=size(i1)
		leni2=size(i2)
	...
	type(Tensor) function Tenproduct_Intname2(T1_,name1,Nimname1,T2_,name2,Nimname2) result(T)
		type(Tensor),intent(in) :: T1_,T2_
		integer,intent(in)::name1(:),name2(:),Nimname1(:),Nimname2(:)
		integer :: i1,i2
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
	...
	type(Tensor) function Tenproduct_name_rename1(T1_,name1,T2_,name2,rename,whichname) result(T)
		...
		if(whichname.eq.1)then
			call setDimName(T1%TenDim,oldName,newName)
		end if
		call dimoperation(T1,(/1,rank1-leni1+1,rank1/))
		T2=T2_.pf.i2
		if(whichname.eq.2)then
			call setDimName(T2%TenDim,oldName,newName)
		end if
		call dimoperation(T2,(/1,1,leni2/))
		T=T1 * T2
		return
	...
	type(Tensor) function Tenproduct_int_name2(T1_,i1,T2_,name2) result(T)
		type(Tensor),intent(in) :: T1_,T2_
		character(len=*),intent(in)::name2
		integer,intent(in)::i1
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
	...
	type(Tensor) function Tenproduct_int_intname(T1_,i1,T2_,name2,DimName2) result(T)
		type(Tensor),intent(in) :: T1_,T2_
		...
		i2=Nameorder(T2_%TenDim,name2,DimName2)
		rank1=T1_%rank
		rank2=T2_%rank
		leni1=size(i1)
		leni2=size(i2)
	...
	type(Tensor) function Tenproduct_int_intname2(T1_,i1,T2_,name2,DimName2) result(T)
		type(Tensor),intent(in) :: T1_,T2_
		integer,intent(in)::i1,name2(:),DimName2(:)
		integer :: i2
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
	...
	type(Tensor) function Tenproduct_name_int2(T1_,name1,T2_,i2) result(T)
		type(Tensor),intent(in) :: T1_,T2_
		character(len=*),intent(in)::name1
		integer,intent(in)::i2
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
	...
	type(Tensor) function Tenproduct_intname_int(T1_,name1,DimName1,T2_,i2) result(T)
		type(Tensor),intent(in) :: T1_,T2_
		...
		i1=Nameorder(T1_%TenDim,name1,DimName1)
		rank1=T1_%rank
		rank2=T2_%rank
		leni1=size(i1)
		leni2=size(i2)
	...
	type(Tensor) function Tenproduct_intname_int2(T1_,name1,DimName1,T2_,i2) result(T)
		type(Tensor),intent(in) :: T1_,T2_
		integer,intent(in)::i2,name1(:),DimName1(:)
		integer :: i1
		if(.not.(if_original_dim(T1_%TenDim).and.if_original_dim(T2_%TenDim))) then
...
!cccccccccccccccc  TensorProduct1Dim  cccccccccccccccccc
	complex*16 function TensorProduct1Dim(T1,T2)
		...
		if(.not.T1%flag)then
			write(*,*)"There is no data in the Tensor,(dot2)"
			call error_stop()
		end if
		TensorProduct1Dim=ZDOTU(N1,T1%Tensor_Data,1,T2%Tensor_Data,1)
		return
	end function
	...
	type(Tensor) function ProductTensor (T1,T2)
		...
		select case (flag)
			...
			case (2)
				...
				if(T2n.eq.1)then![m]*[m,1]case
					ProductTensor%Tensor_Data(1)=ZDOTU(T2m,T1%Tensor_Data,1,T2%Tensor_Data,1)
				else
		...
		case (3)
				...
				if(T1m.eq.1)then![1,m]*[m]case
					ProductTensor%Tensor_Data(1)=ZDOTU(T1n,T1%Tensor_Data,1,T2%Tensor_Data,1)
				else
...
!		combine two index of the Tensor,which is con_index and con_index+1
	type(Tensor) function contract(T1,con_index)
		...
		call storeTenData(contract,T1%Tensor_Data(1:T1%totalData))
...
!		if index2 larger than rank,the function will contract the index of index1 -> rank
	type(Tensor) function contracts(T1,index1,index2)
		...
		call storeTenData(contracts,T1%Tensor_Data(1:T1%totalData))
	...
	type(Tensor) function contractsv(T1,vector)
		...
		call storeTenData(contractsv,T1%Tensor_Data(1:T1%totalData))
...
!		if inde larger than rank ,the function will return no change	
	type(Tensor) function decompose(T1,de_index,inde)
		...
		call storeTenData(decompose,T1%Tensor_Data(1:T1%totalData))
	...
	type(Tensor) function decomposev(T1,vector)
		...
		inde=vector(2)
		newDim=DimDecompose(T1%TenDim,de_index,inde)
		call storeTenData(decomposev,T1%Tensor_Data(1:T1%totalData))
	...
	type(Tensor) function decompose1(T1,de_index)
		...
		call storeTenData(decompose1,T1%Tensor_Data(1:T1%totalData))
	...
	type(Tensor) function decomposeAll(T1)
		...
		call storeTenData(decomposeAll,T1%Tensor_Data(1:T1%totalData))
...
!		other element of every row is input parameter of the function
	type(Tensor) function compose_decompse(T1,Operar)
		...
		call storeTenData(compose_decompse,T1%Tensor_Data(1:T1%totalData))
...
!		other element of every row is input parameter of the function
	...
	subroutine dimOperation2(T,Operat)
		integer,intent(in)::Operat(:)
		type(Tensor),intent(inout) :: T(:)
		integer::i
		do i=1,size(T)
			if(getFlag(T(i)))then
				call dimOperation1(T(i),Operat)
			end if
		end do
		return
	end subroutine
	subroutine dimOperation3(T,Operat)
		integer,intent(in)::Operat(:,:)
		type(Tensor),intent(inout) :: T(:)
		integer::i
		do i=1,size(T)
			if(getFlag(T(i)))then
				call dimOperations(T(i),Operat)
			end if
		end do
		return
	end subroutine
...
!***************** equal_of_Tensor *****************
	logical function equal_of_Tensor(T1,T2)
		...
		l=count(abs(T1%Tensor_data(1:T1%totalData)-T2%Tensor_data(1:T2%totalData)).gt.1d-10)
		if(l.gt.0) then
			equal_of_Tensor=.false.
			return
		end if
...
!*****************  maxElement  *****************
	real*8 function maxElement(T)
		...
		maxElement=maxval(abs(T%Tensor_Data(1:T%totalData)))
		return
	end function
!*****************  maxRealElement  *****************
	real*8 function maxRealElement(T)
		...
		maxRealElement=maxval(real(T%Tensor_Data(1:T%totalData)))
		return
	end function
!*****************  minElement  *****************
	real*8 function minElement(T)
		...
		minElement=minval(abs(T%Tensor_Data(1:T%totalData)))
		return
	end function
!*****************  minRealElement  *****************
	real*8 function minRealElement(T)
		...
		minRealElement=minval(real(T%Tensor_Data(1:T%totalData)))
		return
	end function
!********************************************************************
!make QR decompostion: A = QR, Q is an orthomomal matrix, and R is an upper triangle
...
! 			on output T=U*eye(s)*V
	...
	subroutine SVDcutoff2_test(T,U,s,V,Ncut_) 
		type(Tensor),intent(in)::T
		type(Tensor),intent(out)::U,s,V
		integer,intent(in)::Ncut_
		integer::Ncut
		integer m,n,info,lw,rwdim,i,j
		integer :: ms_max,min_MN,max_MN,totaldata
		type(Dimension) :: T1Dim,T2Dim
		if(T%rank.ne.2) then
			if(.not.T%flag) then
				write(*,*)"ERROR in svd"
		...
		if(not_SVD_flag)then
			allocate(SVD_rw(rwdim))
			allocate(SVD_iw(8*min_MN))
			allocate(SVD_T1data(m,min_MN))
			allocate(SVD_T2data(min_MN,n))
			allocate(SVD_sdata(min_MN)) 
			allocate(SVD_work(lw))
			allocate(SVD_Tdata(totaldata))
			not_SVD_flag=.false.
		else
			if(rwdim.gt.SVD_rwdim) then
				deallocate(SVD_rw)
				allocate(SVD_rw(rwdim))
			end if
			if(min_MN.gt.SVD_min_MN) then
				deallocate(SVD_iw)
				deallocate(SVD_sdata)
				allocate(SVD_iw(8*min_MN))
				allocate(SVD_sdata(min_MN)) 
			end if
			if(lw.gt.SVD_lw) then
				deallocate(SVD_work)
				allocate(SVD_work(lw))
			end if
			if(totaldata.gt.SVD_totaldata) then
				deallocate(SVD_Tdata)
				allocate(SVD_Tdata(totaldata))
			end if
			if(m.gt.SVD_M.or.min_MN.gt.SVD_min_MN)then
				deallocate(SVD_T1data)
				allocate(SVD_T1data(m,min_MN))
			end if
			if(n.gt.SVD_N.or.min_MN.gt.SVD_min_MN)then
				deallocate(SVD_T2data)
				allocate(SVD_T2data(min_MN,n))
			end if
		endif
		SVD_Tdata=T
		call ZGESVD('S','S',m,n,SVD_Tdata,m,SVD_sdata,&
				SVD_T1data,m,SVD_T2data,min_MN,SVD_WORK,lw,SVD_rw,INFO)
		if(info.ne.0) then
			write (*,*) "Error in svd ,info=",info
			call error_stop()
		end if
...
!X is a vector or a matrix,the dimension of which is the same as B
	type(Tensor) function linequ(A,B)
		...
		allocate(IPIV(Na))
		allocate(XData(B%TotalData))
		Xdata=B
		call copyTensor(Adata,A)!the subroutine ZGESV will change A
...
!***************   realTen  **********************
	type(DTensor) function realTen(T)
		type(Tensor),intent(in)::T
		realTen=DbuildTen8(T%TenDim,dreal(T%Tensor_Data(1:T%TotalData)))
		return
	end function	
!***************   imagTen  **********************
	type(DTensor) function imagTen(T)
		type(Tensor),intent(in)::T
		imagTen=DbuildTen8(T%TenDim,dimag(T%Tensor_Data(1:T%TotalData)))
		return
	end function		
!T=dcmplx(T1,T2)
	type(Tensor) function dcmplxTen(DT1,DT2) result(T)
		...
		dim2=.subDim.DT2
		if(.not.(dim1.equ.dim2)) then
			write(*,*)"The dimension of T1 and T2 are not the same,(dcmplxTen)"
			write(*,*)"The program will stop"
			call error_stop()
		end if
		call allocateTensor(T,dim1)
		T%Tensor_Data=dcmplx(DT1%DTensor_Data(1:DgetTotalData(DT1)),DT2%DTensor_Data(1:DgetTotalData(DT2)))
		return
		return
	end function	
!*****************  value  *****************
...
!*****************  Element   *****************
	subroutine Element_subroutine2(i,j,Tdata,LD1,LD2,output)
		integer,intent(in)::i,j,LD1,LD2
		complex*16,intent(in)::Tdata(LD1,LD2)
		complex*16,intent(out)::output
		output=Tdata(i,j)
		return
	end subroutine
	subroutine Element_subroutine3(i,j,k,Tdata,LD1,LD2,LD3,output)
	...
	subroutine Element_subroutine4(i,j,k,l,Tdata,LD1,LD2,LD3,LD4,output)
	...
	complex*16 function Element(T,Tdim)
		...
		if(size(Tdim).ne.T%rank) then
			write(*,*)"ERROR in Element (.i.)"
			call error_stop()
		end if
		if(T%rank.eq.2)then
			...
			call Element_subroutine2(Tdim(1),Tdim(2),T%Tensor_Data,T.dim.1,T.dim.2,Element)
			return
		end if
		if(T%rank.eq.3)then
			...
			call Element_subroutine3(Tdim(1),Tdim(2),Tdim(3),T%Tensor_Data,T.dim.1,T.dim.2,T.dim.3,Element)
			return
		end if
		if(T%rank.eq.4)then
			...
			call Element_subroutine4(Tdim(1),Tdim(2),Tdim(3),Tdim(4),T%Tensor_Data,T.dim.1,T.dim.2,T.dim.3,T.dim.4,Element)
			return
		end if
		if(T%rank.eq.1)then
			if(Tdim(1).gt.T%TotalData)Then
				write(*,*)"Index is larger than the len of Tensor"
				call error_stop()
			end if
			Element=T%Tensor_Data(Tdim(1))
			return
		end if
		inde=addressToIndes(T,Tdim)
		if(inde.gt.T%TotalData)Then
			write(*,*)"Index is larger than the len of Tensor"
			call error_stop()
		end if
...
!	or [inde1_min,inde1_max,inde2_min,inde2_max] output data(inde1_min:inde1_max,inde2_min:inde2_max)
	...
	type(Tensor) function subTen(T,inde)	
		...
		if(size(inde).eq.4) then
			...
			call allocateTensor(subTen,(/dim1,dim2/))
			call subTen_routine1(T%Tensor_Data,subTen%Tensor_data,T.dim.1,T.dim.2,dim1,&
		...
		if(size(inde).eq.2) then
			...
			select case (inde(1))
				case (-1)!output row
					call allocateTensor(subTen,(/dim2/))
					call subTen_routine2(T%Tensor_Data,subTen%Tensor_Data,dim1,dim2,dim2,inde(2),.true.)
				case (-2)!
					call allocateTensor(subTen,(/dim1/))
					call subTen_routine2(T%Tensor_Data,subTen%Tensor_Data,dim1,dim2,dim1,inde(2),.false.)
				case default 
		...
		select case (inde(1))
			case (-1)!output row
				...
				dim2=T.dim.2
				call allocateTensor(subTen,(/dim1,dim2/))
				call subTen_routine3(T%Tensor_Data,subTen%Tensor_Data,T.dim.1,T.dim.2,dim1,&
					dim2,inde(2),inde(3),.true.)
			case (-2)!
				dim1=T.dim.1
				dim2=inde(3)-inde(2)+1
				call allocateTensor(subTen,(/dim1,dim2/))
				call subTen_routine3(T%Tensor_Data,subTen%Tensor_Data,T.dim.1,T.dim.2,dim1,&
					dim2,inde(2),inde(3),.false.)
			case default 
				write(*,*) "no such case in subTen"
				write(*,*)inde
				call error_stop()
	...
	subroutine subTen_routine1(Tensordata,outdata,TLD1,TLD2,OLD1,OLD2,istart,iend,jstart,jend)
		integer,intent(in)::TLD1,TLD2,OLD1,OLD2,istart,iend,jstart,jend
		complex*16,intent(in)::Tensordata(TLD1,TLD2)
		complex*16,intent(inout)::outdata(OLD1,OLD2)
		outdata=Tensordata(istart:iend,jstart:jend)
		return
	end subroutine
	subroutine subTen_routine2(Tensordata,outdata,TLD1,TLD2,OLD1,iindex,flag)
		integer,intent(in)::TLD1,TLD2,OLD1,iindex
		complex*16,intent(in)::Tensordata(TLD1,TLD2)
		complex*16,intent(inout)::outdata(OLD1)
	...
	subroutine subTen_routine3(Tensordata,outdata,TLD1,TLD2,OLD1,OLD2,istart,iend,flag)
...
!	combination(T1,T2):a [...,m,n,l+1] matrix
	type(Tensor) function combination(T1,T2)
		...
		if(T1%rank.eq.T2%rank) then
			...
			newdata(:total1)=T1!%Tensor_Data(1:T1%Totaldata)
			newdata(total1+1:)=T2!%Tensor_Data(1:T2%Totaldata)
...
!******************  modify the data in the Tensor  **************
	subroutine modifyTen_array_subroutine1(i,Tdata,LenT,value,LenV)
		integer,intent(in)::i(2),LenT,LenV
		complex*16,intent(inout)::Tdata(LenT)
		complex*16,intent(in)::value(LenV)
		Tdata(i(1):i(2))=value
		return
	end subroutine
	subroutine modifyTen_val_subroutine2(i,j,Tdata,LD1,LD2,value)
		integer,intent(in)::i,j,LD1,LD2
		complex*16,intent(inout)::Tdata(LD1,LD2)
		complex*16,intent(in)::value
		Tdata(i,j)=value
		return
	end subroutine
	subroutine modifyTen_array_subroutine2(i,j,Tdata,LD1,LD2,value,LDV1,LDV2)
		integer,intent(in)::i(2),j(2),LD1,LD2,LDV1,LDV2
		complex*16,intent(inout)::Tdata(LD1,LD2)
		complex*16,intent(in)::value(LDV1,LDV2)
		Tdata(i(1):i(2),j(1):j(2))=value
		return
	end subroutine
	subroutine modifyTen_val_subroutine3(i,j,l,Tdata,LD1,LD2,LD3,value)
		integer,intent(in)::i,j,l,LD1,LD2,LD3
		complex*16,intent(inout)::Tdata(LD1,LD2,LD3)
		complex*16,intent(in)::value
		Tdata(i,j,l)=value
		return
	end subroutine
	subroutine modifyTen_array_subroutine3(i,j,l,Tdata,LD1,LD2,LD3,value,LDV1,LDV2,LDV3)
		integer,intent(in)::i(2),j(2),l(2),LD1,LD2,LD3,LDV1,LDV2,LDV3
		complex*16,intent(inout)::Tdata(LD1,LD2,LD3)
		complex*16,intent(in)::value(LDV1,LDV2,LDV3)
		Tdata(i(1):i(2),j(1):j(2),l(1):l(2))=value
		return
	end subroutine
	subroutine modifyTen_val_subroutine4(i,j,k,l,Tdata,LD1,LD2,LD3,LD4,value)
		integer,intent(in)::i,j,k,l,LD1,LD2,LD3,LD4
		complex*16,intent(inout)::Tdata(LD1,LD2,LD3,LD4)
		complex*16,intent(in)::value
		Tdata(i,j,k,l)=value
		return
	end subroutine
	subroutine modifyTen_array_subroutine4(i,j,k,l,Tdata,LD1,LD2,LD3,LD4,value,LDV1,LDV2,LDV3,LDV4)
		integer,intent(in)::i(2),j(2),k(2),l(2),LD1,LD2,LD3,LD4,LDV1,LDV2,LDV3,LDV4
		complex*16,intent(inout)::Tdata(LD1,LD2,LD3,LD4)
		complex*16,intent(in)::value(LDV1,LDV2,LDV3,LDV4)
...
! modify the data in dimen of the Tensor
	subroutine modifyTen_val(Ten,dimen,val)
		...
		if(Ten%rank.eq.1) then
			if(dimen(1).gt.Ten%TotalData)Then
				write(*,*)"Index is larger than the len of Tensor"
				call error_stop()
			end if
			Ten%Tensor_Data(dimen(1))=val
			return
		end if
		if(Ten%rank.eq.2) then
			...
			call modifyTen_val_subroutine2(dimen(1),dimen(2),Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,val)
			return
		end if
		if(Ten%rank.eq.3) then
			...
			call modifyTen_val_subroutine3(dimen(1),dimen(2),dimen(3),Ten%Tensor_Data,&
							Ten.dim.1,Ten.dim.2,Ten.dim.3,val)
			return
		end if
		if(Ten%rank.eq.4) then
			...
			call modifyTen_val_subroutine4(dimen(1),dimen(2),dimen(3),dimen(4),Ten%Tensor_Data,&
							Ten.dim.1,Ten.dim.2,Ten.dim.3,Ten.dim.4,val)
			return
		end if
	...
	subroutine modifyTen_val1(Ten,ith,val)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::ith
		complex*16,intent(in)::val
		Ten%Tensor_Data(ith)=val
		return
	end subroutine
	subroutine modifyTen_all(Ten,Allval)
		type(Tensor),intent(inout)::Ten
		complex*16,intent(in)::Allval(*)
		call ZCOPY(Ten%TotalData,Allval,1,Ten%Tensor_Data,1)
		return
	end subroutine
	subroutine modifyTen_array_val1(Ten,dimen1,val)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(2)
		complex*16,intent(in)::val(:)
		call modifyTen_array_subroutine1(dimen1,Ten%Tensor_Data,Ten%totalData,val,size(val))
		return
	end subroutine
	subroutine modifyTen_array_val2(Ten,dimen1,dimen2,val)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:)
		complex*16,intent(in)::val(:,:)
		if(Ten%rank.ne.2) then
			write(*,*)"ERROR in modifyTen"
			call error_stop()
		end if
		call modifyTen_array_subroutine2(dimen1,dimen2,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						val,size(val,1),size(val,2))
		return
	end subroutine
	subroutine modifyTen_array_val2_2(Ten,dimen1,dimen2,val)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:)
		complex*16,intent(in)::val(:)
		integer::row,col
		...
		row=dimen1(2)-dimen1(1)+1
		col=dimen2(2)-dimen2(1)+1
		call modifyTen_array_subroutine2(dimen1,dimen2,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						val,row,col)
		return
	end subroutine
	subroutine modifyTen_array_val3(Ten,dimen1,dimen2,dimen3,val)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:)
		complex*16,intent(in)::val(:,:,:)
		if(Ten%rank.ne.3) then
			write(*,*)"ERROR in modifyTen"
			call error_stop()
		end if
		call modifyTen_array_subroutine3(dimen1,dimen2,dimen3,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						Ten.dim.3,val,size(val,1),size(val,2),size(val,3))
		return
	end subroutine
	subroutine modifyTen_array_val3_2(Ten,dimen1,dimen2,dimen3,val)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:)
		complex*16,intent(in)::val(:)
		integer::row,col,kcol
		if(Ten%rank.ne.3) then
			write(*,*)"ERROR in modifyTen"
			call error_stop()
		end if
		row=dimen1(2)-dimen1(1)+1
		col=dimen2(2)-dimen2(1)+1
		kcol=dimen3(2)-dimen3(1)+1
		call modifyTen_array_subroutine3(dimen1,dimen2,dimen3,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						Ten.dim.3,val,row,col,kcol)
		return
	end subroutine
	
	subroutine modifyTen_array_val4(Ten,dimen1,dimen2,dimen3,dimen4,val)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:),dimen4(:)
		complex*16,intent(in)::val(:,:,:,:)
		if(Ten%rank.ne.4) then
			write(*,*)"ERROR in modifyTen"
			call error_stop()
		end if
		call modifyTen_array_subroutine4(dimen1,dimen2,dimen3,dimen4,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						Ten.dim.3,Ten.dim.4,val,size(val,1),size(val,2),size(val,3),size(val,4))
		return
	end subroutine
	subroutine modifyTen_array_val4_2(Ten,dimen1,dimen2,dimen3,dimen4,val)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:),dimen4(:)
		complex*16,intent(in)::val(:)
		integer::row,col,kcol,col4
		if(Ten%rank.ne.4) then
			write(*,*)"ERROR in modifyTen"
			call error_stop()
		end if
		...
		col=dimen2(2)-dimen2(1)+1
		kcol=dimen3(2)-dimen3(1)+1
		col4=dimen4(2)-dimen4(1)+1
		call modifyTen_array_subroutine4(dimen1,dimen2,dimen3,dimen4,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						Ten.dim.3,Ten.dim.4,val,row,col,kcol,col4)
		return
	end subroutine
	
	subroutine modifyTen_Tensor_all(Ten,dimen1,Ten2)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(2)
		type(Tensor),intent(in)::Ten2
		integer::totaldata
		totaldata=dimen1(2)-dimen1(1)+1
		call zcopy(totaldata,Ten2%Tensor_Data,1,Ten%Tensor_Data(dimen1(1):dimen1(2)),1)
		return
	end subroutine
	
	subroutine modifyTen_Tensor_val1(Ten,dimen1,Ten2,dimen21)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(2),dimen21(2)
		type(Tensor),intent(in)::Ten2
		Ten%Tensor_Data(dimen1(1):dimen1(2))=Ten2%Tensor_Data(dimen21(1):dimen21(2))
		return
	end subroutine
	subroutine modifyTen_Tensor_val2(Ten,dimen1,dimen2,Ten2,dimen21,dimen22)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen21(:),dimen22(:)
		type(Tensor),intent(in)::Ten2
		if(Ten%rank.ne.2) then
			write(*,*)"ERROR in modifyTen"
			call error_stop()
		end if
		call modifyTen_Tensor_val2_subroutine(dimen1,dimen2,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						dimen21,dimen22,Ten2%Tensor_Data,Ten2.dim.1,Ten2.dim.2)
		return
	end subroutine
	subroutine modifyTen_Tensor_val2_2(Ten,dimen1,dimen2,Ten2)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:)
		type(Tensor),intent(in)::Ten2
		integer::dimen21(2),dimen22(2)
		if(Ten%rank.ne.2) then
			write(*,*)"ERROR in modifyTen"
			call error_stop()
		end if
		dimen21=(/1,Ten2.dim.1/)
		dimen22=(/1,Ten2.dim.2/)
		call modifyTen_Tensor_val2_subroutine(dimen1,dimen2,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						dimen21,dimen22,Ten2%Tensor_Data,Ten2.dim.1,Ten2.dim.2)
		return
	end subroutine
	subroutine modifyTen_Tensor_val3(Ten,dimen1,dimen2,dimen3,Ten2,dimen21,dimen22,dimen23)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:),dimen21(:),dimen22(:),dimen23(:)
		type(Tensor),intent(in)::Ten2
		if(Ten%rank.ne.3) then
			write(*,*)"ERROR in modifyTen"
			call error_stop()
		end if
		call modifyTen_Tensor_val3_subroutine(dimen1,dimen2,dimen3,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						Ten.dim.3,dimen21,dimen22,dimen23,Ten2%Tensor_Data,Ten2.dim.1,Ten2.dim.2,Ten2.dim.3)
		return
	end subroutine
	subroutine modifyTen_Tensor_val3_2(Ten,dimen1,dimen2,dimen3,Ten2)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:)
		type(Tensor),intent(in)::Ten2
		integer::dimen21(2),dimen22(2),dimen23(2)
		if(Ten%rank.ne.3) then
			write(*,*)"ERROR in modifyTen"
			call error_stop()
		end if
		dimen21=(/1,Ten2.dim.1/)
		dimen22=(/1,Ten2.dim.2/)
		dimen23=(/1,Ten2.dim.3/)
		call modifyTen_Tensor_val3_subroutine(dimen1,dimen2,dimen3,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						Ten.dim.3,dimen21,dimen22,dimen23,Ten2%Tensor_Data,Ten2.dim.1,Ten2.dim.2,Ten2.dim.3)
		return
	end subroutine
	subroutine modifyTen_Tensor_val4(Ten,dimen1,dimen2,dimen3,dimen4,Ten2,dimen21,dimen22,dimen23,dimen24)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:),dimen4(:),dimen21(:),dimen22(:),dimen23(:),dimen24(:)
		type(Tensor),intent(in)::Ten2
		if(Ten%rank.ne.4) then
			write(*,*)"ERROR in modifyTen"
			call error_stop()
		end if
		call modifyTen_Tensor_val4_subroutine(dimen1,dimen2,dimen3,dimen4,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						Ten.dim.3,Ten.dim.4,dimen21,dimen22,dimen23,dimen24,Ten2%Tensor_Data,Ten2.dim.1,Ten2.dim.2,&
						Ten2.dim.3,Ten2.dim.4)
		return
	end subroutine
	subroutine modifyTen_Tensor_val4_2(Ten,dimen1,dimen2,dimen3,dimen4,Ten2)
		type(Tensor),intent(inout)::Ten
		integer,intent(in)::dimen1(:),dimen2(:),dimen3(:),dimen4(:)
		type(Tensor),intent(in)::Ten2
		integer::dimen21(2),dimen22(2),dimen23(2),dimen24(2)
		if(Ten%rank.ne.4) then
			write(*,*)"ERROR in modifyTen"
			call error_stop()
		end if
		...
		dimen22=(/1,Ten2.dim.2/)
		dimen23=(/1,Ten2.dim.3/)
		dimen24=(/1,Ten2.dim.4/)
		call modifyTen_Tensor_val4_subroutine(dimen1,dimen2,dimen3,dimen4,Ten%Tensor_Data,Ten.dim.1,Ten.dim.2,&
						Ten.dim.3,Ten.dim.4,dimen21,dimen22,dimen23,dimen24,Ten2%Tensor_Data,Ten2.dim.1,Ten2.dim.2,&
						Ten2.dim.3,Ten2.dim.4)
		return
	end subroutine
	subroutine modifyTen_Tensor_val2_subroutine(dim1,dim2,Data1,LD1,LD2,dim3,dim4,data2,LD3,LD4)
		integer,intent(in)::LD1,LD2,LD3,LD4,dim1(2),dim2(2),dim3(2),dim4(2)
		complex*16,intent(inout)::Data1(LD1,LD2)
		complex*16,intent(in)::Data2(LD3,LD4)
		Data1(dim1(1):dim1(2),dim2(1):dim2(2))=Data2(dim3(1):dim3(2),dim4(1):dim4(2))
		return
	end subroutine
	
	subroutine modifyTen_Tensor_val3_subroutine(dim1,dim2,dim3,Data1,LD1,LD2,LD3,dim4,dim5,dim6,data2,LD4,LD5,LD6)
		integer,intent(in)::LD1,LD2,LD3,LD4,LD5,LD6,dim1(2),dim2(2),dim3(2),dim4(2),dim5(2),dim6(2)
		complex*16,intent(inout)::Data1(LD1,LD2,LD3)
		complex*16,intent(in)::Data2(LD4,LD5,LD6)
		Data1(dim1(1):dim1(2),dim2(1):dim2(2),dim3(1):dim3(2))=Data2(dim4(1):dim4(2),dim5(1):dim5(2),dim6(1):dim6(2))
		return
	end subroutine
	subroutine modifyTen_Tensor_val4_subroutine(dim11,dim12,dim13,dim14,Data1,LD11,LD12,LD13,LD14,dim21,&
											dim22,dim23,dim24,data2,LD21,LD22,LD23,LD24)
		integer,intent(in)::LD11,LD12,LD13,LD14,LD21,LD22,LD23,LD24
		integer,intent(in)::dim11(2),dim12(2),dim13(2),dim14(2),dim21(2),dim22(2),dim23(2),dim24(2)
		complex*16,intent(inout)::Data1(LD11,LD12,LD13,LD14)
		complex*16,intent(in)::Data2(LD21,LD22,LD23,LD24)
		Data1(dim11(1):dim11(2),dim12(1):dim12(2),dim13(1):dim13(2),dim14(1):dim14(2))=Data2(&
					dim21(1):dim21(2),dim22(1):dim22(2),dim23(1):dim23(2),dim24(1):dim24(2))
		return
	end subroutine
...
!			
	type(Tensor) function SortTen(T,realpart,increase,inde)
		type(Tensor),intent(in)::T
		logical,intent(in)::realpart,increase
		integer,allocatable,optional,intent(out):: inde(:)
		if(T%rank.ne.1) then
			write(*,*)"Sort,only rank=1 is allowed"
			write(*,*)"stop"
			call error_stop()
		end if
		call copyTensor(SortTen%Tensor_Data,T)
		if(present(inde)) then
			call sort(SortTen%Tensor_Data,inde,realpart,increase)
		else
...
!**********************************************************************************	
		if(proID.eq.ID1) then
			call mpi_send(Ten1%Tensor_data,2*Ten1%totalData,MPI_complex,ID2,tag,MPI_Comm,ierr)
		end if
		if(proID.eq.ID2) then
			...
			!allocate(Ten2%Tensor_data(Ten2%totalData))
			call mpi_recv(Ten2%Tensor_data,2*Ten2%totalData,MPI_complex,ID1,tag,MPI_Comm,istatus,ierr)
		end if
!**********************************************************************************	
		return
	end subroutine
...
!**********************************************************************************	
		...
		call MPI_BCAST(Ten1%Tensor_data,2*Ten1%totalData,MPI_complex,ID,mpi_comm,ierr)
!**********************************************************************************	
		return
	end subroutine
