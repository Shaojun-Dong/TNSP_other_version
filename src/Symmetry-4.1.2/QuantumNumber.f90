...
module QuantumNumber_Type
	...
	contains	
		...
		procedure,public::NonZeroDeg
		procedure,public::NonZeroDegIndex
...
contains
	...
	type(QuanNum) function NonZeroDeg(QN)
		class(QuanNum),intent(in)::QN
		real*4,allocatable::Qnum(:)
		integer,allocatable::deg(:)
		integer::oldlenQN,Rule,Arrow,lenQN
		...
		if(QN%ifzeroDeg())then
			...
			Qnum=QN%QuanNum(ith:jth)
			deg=QN%degeneracy(ith:jth)
			call NonZeroDeg%empty()
			call NonZeroDeg%setQN(Qnum)
			call NonZeroDeg%setDeg(deg)
			call NonZeroDeg%setRule(Rule)
			call NonZeroDeg%setFermiArrow(Arrow)
		else
			NonZeroDeg=QN
		end if
		return
	...
	function NonZeroDegIndex(QN)result(Degindex)
