module usefull_function
	...
	integer,private,save::min_len_of_char=300
	interface delta
...
! the line element of a is the max
	...
	character(len=(max(len(w1)+len(w2),min_len_of_char))) function charAdd(w1,w2)
		character(len=*),intent(in)::w1,w2
		charAdd=(trim(adjustl(w1)))//(trim(adjustl(w2)))
		return
	end function
	...
	character(len=(max(len(w1),min_len_of_char))) function charAddint(w1,inte)
	...
	character(len=(max(len(w1),min_len_of_char))) function intAddchar(inte,w1)
		character(len=*),intent(in)::w1
		integer,intent(in)::inte
		character*500::temp
		write(temp,'(I4)')inte
