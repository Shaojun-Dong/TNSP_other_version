...
!********************* print_dim *********************            
	subroutine TDprint(T,easyprint)
		type(Tensor) :: T
		logical,optional,intent(in)::easyprint
		if(T%flag) then!if1
			if(present(easyprint))then!if2
				call Dprint1(T%TenDim)
			else
				write(*,*) "*** START ***"
				write(*,*) "The rank of the Tensor is"
				write(*,*) T%rank
...
! if the index of element that is modify,then allocate a new data 
	subroutine modifyTen_val2(Ten,inde,val)
		...
		if(inde.le.totalData) then
			...
			call copyTensor(Tdata,Ten)
			if(Ten%flag)then
				if(size(Ten%Tensor_Data).lt.inde)then
...
! modify all the data in the Tensor
	subroutine modifyTen_dat(Ten,val)
		...
		call ZCOPY(size(val),val,1,Ten%Tensor_Data,1)
		return
	end subroutine
! modify all the data in the Tensor
