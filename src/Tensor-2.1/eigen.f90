...
!  dimension_of_phi_int: integer (:) , the  dimension of the state
		...
		module procedure eigenvalue_H_phi3!(H_phi,H,dimension_of_phi,WHICH,num_of_eig,outvector_)
		...
		module procedure eigenvalue_H_phi_MPI1!(H_phi,H,dimension_of_phi,WHICH,num_of_eig,ncv,tol,maxitr,cpuInfo,outvector_)
!H_phi : call H_phi(H,state,cpuInfo)
!cpuInfo(1),proid
!cpuInfo(2),pronum
!cpuInfo(3),main_cpu
!cpuInfo(4),ierr
!    eigenvalue_H_phi_MPI1 the energe output is OK, but should check the state. although .H.state * H * state
!  give the right value, but .H.state * O * state may not, I don't konw why. And there is no error in my 
!  subroutine. I modify the location of the code
!		call mpi_init(ierr)
!		call mpi_comm_rank(mpi_comm_world,proID,ierr)
!		call mpi_comm_size(mpi_comm_world,proNum,ierr )
!		call set_cpu_info(proID,proNum,ierr)
!  do not put them in the beginning, then the program gose right. I don't konw why!!!!!
	end interface
!*************************************************************************************
...
!  dimension_of_phi: type(Dimension), the dimension of the state
!  dimension_of_phi_int: integer (:) , the  dimension of the state
		module procedure eigenvalue_H_phi_array1!(H_phi,H,WHICH,num_of_eig,ncv,tol,maxitr,dimension_of_phi,outvector_)
		module procedure eigenvalue_H_phi_array2!(H_phi,H,WHICH,num_of_eig,dimension_of_phi,outvector_)
		module procedure eigenvalue_H_phi_array3!(H_phi,H,WHICH,num_of_eig,dimension_of_phi_int,outvector_)
...
!cpuInfo(3),main_cpu
!cpuInfo(4),ierr
!only the main_cpu will run the code, other with not until the calling call H_phi(H,phi)
	function eigenvalue_H_phi_MPI1(H_phi,H,dimension_of_phi,WHICH,num_of_eig,ncv,cpuInfo,outvector_) result(resultlink)
				type(Tensorlink)::resultlink
		type(Tensorlink),intent(in)::H
		external::H_phi
		Character*2,intent(in)::WHICH
		logical,optional,intent(in)::outvector_
		logical::outvector
		integer,intent(in)::num_of_eig,ncv,cpuInfo(4)
		type(Dimension),intent(inout)::dimension_of_phi
		real*8::tol,maxitr
		complex*16,allocatable::A(:),v(:,:),resid(:),d(:),Cdata(:),workdata(:)
		complex*16,allocatable::workd(:),workev(:),workl(:)
		...
		type(Tensornode),pointer::res_eigvalue,res_eigenvector
		type(Tensor)::phi
		if(cpuInfo(1).eq.cpuInfo(3))then
			if(present(outvector_))then
				outvector=outvector_
			else
				outvector=.false.
		...
		do while(continu)
			...
			call MPI_BCAST(ido,1,MPI_integer,cpuinfo(3),MPI_COMM_WORLD,cpuinfo(4))	
			if (ido .eq. -1 .or. ido .eq. 1 ) then
				if(cpuInfo(1).eq.cpuInfo(3))then
					phi%Tensor_Data=workd(ipntr(1):ipntr(1)+n-1)
				end if
				call H_phi(H,phi,cpuInfo)
				if(cpuInfo(1).eq.cpuInfo(3))then
					do i=0,n-1
						workd(ipntr(2)+i)=phi%Tensor_Data(i+1)
					end do
		...
		if(cpuInfo(1).ne.cpuInfo(3)) return
		dimension_of_phi=.subdim.phi
		if(ido.ne.99)then
			write(*,*)"znaupd do not success"
			write(*,*)info
