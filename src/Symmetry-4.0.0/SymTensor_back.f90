...
!**************************************************************************
	type(SymTensor) function divide_real8(T1,num) result(Res)
		...
		do i=1,T1%getTotalData()
			if(T1%block(i)%getFlag())then
				...
				if(Res%block(i)%getType().ne.classtype)then
					call writemess('ERROR in (/), class type error',-1)
					call error_stop
				end if
			end if
		end do
	...
	subroutine ProductTensorRoutine(Res,T1,T2)
		...
		if(T1%getTotalBlock().eq.0) then
			call writemess("ERROR in (*)",-1)
			call writemess("Tensor is no Block in the first Tensor",-1)
			call writemess("stop",-1)
			call error_stop()
		end if
		if(T2%getTotalBlock().eq.0) then
			call writemess("ERROR in (*)",-1)
			call writemess("Tensor is no Block in the second Tensor",-1)
			call writemess("stop",-1)
			call error_stop()
		end if	
...
!*****************************************************************************
	...
	subroutine choosSameQN(outQN,QN1,QN2)
		...
		if(lenQ.eq.0)then
			call writemess(' ERROR in choosSameQN, there is no QN')
			call error_stop
		end if
...
!Do not keep the Tesnor name
	...
	subroutine SVDSavingSingleValueDeg_(S,Newdeg,NumSave_,LD1,LD2,Flag)
	...
	subroutine SVDSavingSingleValueDegValue_(S,Newdeg,minNumSave,NumSave,maxValue,VType,LD1,LD2)! unknow error
		...
		do i=1,NumSave
			...
			if(VType.equ.'sum2')then
				...
				if(first)then
					...
					if( (( AllSdata(i)/sumValue ).le.maxValue).and.(i.ge.minNumSave)) exit
				end if
			else if(VType.equ.'sum')then
	...
	subroutine sortDataReal4(a,inde,increase)
