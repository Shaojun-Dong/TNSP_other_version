...
!			6.After using the type of Tensorlink or Tensorlist,clean it when they are useless,
!		they are pointers and will not be clean after any subroutine or function,until one call
!		the cleanlink of cleanlist subroutine
...
!the interface below is a subroutine, is the same function as LQ
...
!the interface below is a subroutine , is the same function as QR
...
!
	...
	interface eigen
		module procedure eigen_all!(H,ifvec),ifvec=.true.,output eigenvectors
		module procedure eigen_all_no_vec!(H),output eigenvalue only to a link
...
!when use the function below,test them first
		...
		module procedure eigen4!(T,s0,num_of_ei),outvector=.false.
		...
		module procedure eigen_max_min4!(T,WHICH,num_of_eig),outvector=.false.
...
!  return a matrix of diag[s1,s2,s3,...s_{max},0,0...]	,a m*n matrix
		module procedure eye_com!input s(:) is complex*16,a m*n matrix
		module procedure eye_real!input s(:) is real*8,a m*n matrix
		module procedure eye_com2!input s(:) is complex*16
		module procedure eye_real2!input s(:) is real*8
...
!********************************************************************
	...
	type(Tensorlink) function QR(T) result(res)
		type(Tensor),intent(in)::T
...
!	T=(res.i.1)*(res.i.2)
!********************************************************************
	type(Tensorlink) function LQ(T) result(res)
		type(Tensor),intent(in)::T
		complex*16,allocatable::  work(:),tau(:),Tdata(:,:)
		Type(Tensor)::v,vv,identity
		type(Tensornode),pointer::respointer
...
!result:[eigenvalues]->[eigenvectors]
	type(Tensorlink) function eigen_all(H,ifvec)
	...
	type(Tensorlink) function eigen_all_no_vec(H)
		type(Tensor),intent(in) ::H
		eigen_all_no_vec=eigen_all(H,.false.)
		return
	end function
!********************************************************
...
!	if outvector=.false. no eigenvectors
	 function eigen1(T,s0,num_of_eig,ncv,tol,outvector) result(resultlink)
	...
	function eigen2(T,s0,num_of_eig,ncv,outvector) result(resultlink)
	 	type(Tensorlink)::resultlink
		type(Tensor),intent(in)::T
		complex*16,intent(in)::s0
		real*8::tol
	 ...
	 function eigen3(T,s0,num_of_eig,outvector) result(resultlink)
	 	type(Tensorlink)::resultlink
		type(Tensor),intent(in)::T
		complex*16,intent(in)::s0
	...
	function eigen4(T,s0,num_of_eig)  result(resultlink)!outvector=.ture.
		type(Tensorlink)::resultlink
		type(Tensor),intent(in)::T
		complex*16,intent(in)::s0
		integer,intent(in)::num_of_eig
		...
		resultlink=eigen1(T,s0,num_of_eig,ncv,tol,.false.)
		return
	end function
!********************************************************
...
!          'LI' -> want the NEV eigenvalues of largest imaginary part.
!          'SI' -> want the NEV eigenvalues of smallest imaginary part.
	function eigen_max_min1(T,WHICH,num_of_eig,ncv,tol,outvector) result(resultlink)
		type(Tensorlink)::resultlink
		type(Tensor),intent(in)::T
	...
	function eigen_max_min2(T,WHICH,num_of_eig,ncv,outvector)  result(resultlink)
		type(Tensorlink)::resultlink
		type(Tensor),intent(in)::T
		Character*2,intent(in)::WHICH
		logical,intent(in)::outvector
	...
	function eigen_max_min3(T,WHICH,num_of_eig,outvector)  result(resultlink)
		type(Tensorlink)::resultlink
		type(Tensor),intent(in)::T
		Character*2,intent(in)::WHICH
		logical,intent(in)::outvector
	...
	function eigen_max_min4(T,WHICH,num_of_eig)  result(resultlink)!outvector=.ture.
		type(Tensorlink)::resultlink
		type(Tensor),intent(in)::T
		Character*2,intent(in)::WHICH
		integer,intent(in)::num_of_eig
		...
		resultlink=eigen_max_min1(T,WHICH,num_of_eig,ncv,tol,.false.)
		return
	end function
!*****************  eye  *****************
