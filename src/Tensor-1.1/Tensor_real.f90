...
!			define DcopyLinkhead as (=) ,not DcopyLink . So when using link or list,
...
!DLQ is a function that return a DTensorlink
!the interface below is a subroutine, is the same function as DLQ
...
!DQR is a function that return a DTensorlink 
!the interface below is a subroutine , is the same function as DQR
...
!
	...
	interface eigen
		module procedure Deigen_all!(H,ifvec),ifvec=.true.,output eigenvectors
		module procedure Deigen_all_no_vec!(H),output eigenDvalue only to a link
!		the same as eig,but it is a function and Deng is a subroutine

!eigenDvalue near some Dvalue,say s0
!ncv is the Number of Lanczos vectors to be generated.2 <= NCV-num_of_eig and NCV <= N		
!tol:Stopping criteria
	!	module procedure Deigen1!(T,s0,num_of_eig,ncv,tol,outtype)
	!	module procedure Deigen2!(T,s0,num_of_eig,outtype)
	!	module procedure Deigen3!(T,s0,num_of_eig,),outtype=1
!	if outtype=1 eigenDvalues
!	if outtype=2 eigenDvalues and eigenvectors
!	if outtype=3 eigenDvalues,but using |T-s0|^2 to find the eigenDvalue near s0
!	if outtype=4 eigenDvalues and eigenvectors,but using |T-s0|^2 to find the eigenDvalue near s0		
		
	!	module procedure Deigen_max_min1!(T,WHICH,num_of_eig,ncv,tol,outvector)
	!	module procedure Deigen_max_min2!(T,WHICH,num_of_eig,outvector)
	!	module procedure Deigen_max_min3!(T,WHICH,num_of_eig),outvector=.false.
!				WHICH   Character*2.  (INPUT)
!          'LM' -> want the NEV eigenDvalues of largest magnitude.
!          'SM' -> want the NEV eigenDvalues of smallest magnitude.
!          'LR' -> want the NEV eigenDvalues of largest real part.
!          'SR' -> want the NEV eigenDvalues of smallest real part.
!          'LI' -> want the NEV eigenDvalues of largest imaginary part.
!          'SI' -> want the NEV eigenDvalues of smallest imaginary part.
	end interface
...
!  return a matrix of diag[s1,s2,s3,...s_{max},0,0...]	,a m*n matrix
		module procedure Deye_real!input s(:) is real*8,a m*n matrix
		module procedure Deye_real2!input s(:) is real*8
...
!			A=(res.i.1)*(res.i.2)
!			
!			DQRdecomposition: subroutine type of DQR
!
!		DLQ:
...
!			A=(res.i.1)*(res.i.2)
!
!			DLQdecomposition: subroutine type of DLQ
!
!		linear equations
...
!********************************************************************
	...
	type(DTensorlink) function DQR(T) result(res)
		type(DTensor),intent(in)::T
...
!	T=(res.i.1)*(res.i.2)
!********************************************************************
	type(DTensorlink) function DLQ(T) result(res)
		type(DTensor),intent(in)::T
		real*8,allocatable::  work(:),tau(:),Tdata(:,:)
		Type(DTensor)::v,vv,identity
		type(DTensornode),pointer::respointer
...
! T=(link.i.1)*(eye(link.i.2))*(link.i.3)
	type(DTensorlink) function Dsvd_no_cut(T) result(res)
		...
		allocate(work(lw))
		Tdata=T
		call DGESDD('S',m,n,TData,m,sdata,T1data,m,T2data,n,WORK,lw,iw,INFO)
		if(info.ne.0) then
...
!!! The inverse of a matrix: the input tensor should be a square matrix 
	type(Dtensor) function Dinverse(T)
		type(DTensor),intent(in) :: T
		type(DTensor):: U,s,V
		real*8,allocatable  :: newsdata(:)
		integer :: M,N,i
		M = T.dim.1
		N = T.dim.2
		if(DgetRank(T).ne.2 .and. M/=N) then
			write(*,*)"ERROR in calculating the inverse of a Dtensor"
			write(*,*)"input Tensor should be a square matrix"
			stop
		endif
		call Dsvddecomposition(T,U,s,V)
		allocate(newsdata(N))
		do i=1,M
			if((s.i.i)<zero_num) then
				write(*,*) 'Warning:'
		...
		enddo
		Dinverse=(.p.V)*Deye(newsdata)*(.p.U)
		return
	end function
...
!result:[eigenDvalues,realpart]->[eigenDvalues,imagpart]->[eigenvectors]
	type(DTensorlink) function Deigen_all(H,ifvec)
	...
	type(DTensorlink) function Deigen_all_no_vec(H)
		type(DTensor),intent(in) ::H
		Deigen_all_no_vec=Deigen_all(H,.false.)
		return
	end function
!*****************  Deye  *****************
...
!	output T1'*T2
! 	input Tensor should be in its original dimenison,there is no contract on it
	type(DTensor) function DTenproduct(T1_,T2_,i1_,i2_) result(T)
		type(DTensor),intent(in) :: T1_,T2_
