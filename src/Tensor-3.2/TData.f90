...
!end subroutine!
	...
	public::public_allocate_TData
	interface public_allocate_TData
		module procedure allocateData1
		module procedure allocateData2
		module procedure allocateData3
	end interface
...
!**************************************************************************************************************	
	...
	subroutine intmaxTData(outvalue,T)
		...
		select case(T%ClassType)
			case (1)
				outvalue=maxval(T%idata)
			case (2)
				outvalue=maxval(T%sdata)
			case (3)
				outvalue=maxval(T%ddata)
			case (4)
				outvalue=maxval(abs(T%cdata))
			case (5)
				outvalue=maxval(cdabs(T%zdata))
		end select
		return
	end subroutine
	subroutine intminTData(outvalue,T)
		...
		select case(T%ClassType)
			case (1)
				outvalue=minval(T%idata)
			case (2)
				outvalue=minval(T%sdata)
			case (3)
				outvalue=minval(T%ddata)
			case (4)
				outvalue=minval(abs(T%cdata))
			case (5)
				outvalue=minval(cdabs(T%zdata))
		end select
		return
	end subroutine
	subroutine intmaxrealTData(outvalue,T)
		...
		select case(T%ClassType)
			...
			case (3)
				outvalue=maxval(T%ddata)
			case (4)
				outvalue=maxval(real(T%cdata,kind=4))
			case (5)
				outvalue=maxval(dble(T%zdata))
		end select
		return
	end subroutine
	subroutine intminrealTData(outvalue,T)
		...
		select case(T%ClassType)
			...
			case (3)
				outvalue=minval(T%ddata)
			case (4)
				outvalue=minval(real(T%cdata,kind=4))
			case (5)
				outvalue=minval(dble(T%zdata))
		end select
		return
	end subroutine
	subroutine intmaximagTData(outvalue,T)
		...
		select case(T%ClassType)
			...
			case (3)
				outvalue=0
			case (4)
				outvalue=maxval(aimag(T%cdata))
			case (5)
				outvalue=maxval(dimag(T%zdata))
		end select
		return
	end subroutine
	subroutine intminimagTData(outvalue,T)
		...
		select case(T%ClassType)
			...
			case (3)
				outvalue=0
			case (4)
				outvalue=minval(aimag(T%cdata))
			case (5)
				outvalue=minval(dimag(T%zdata))
		end select
		return
	end subroutine
	subroutine intmaxabsTData(outvalue,T)
		...
		select case(T%ClassType)
			case (1)
				outvalue=maxval(abs(T%idata))
			case (2)
				outvalue=maxval(abs(T%sdata))
			case (3)
				outvalue=maxval(dabs(T%ddata))
			case (4)
				outvalue=maxval(abs(T%cdata))
			case (5)
				outvalue=maxval(cdabs(T%zdata))
	...
	subroutine intminabsTData(outvalue,T)
		...
		select case(T%ClassType)
			case (1)
				outvalue=minval(abs(T%idata))
			case (2)
				outvalue=minval(abs(T%sdata))
			case (3)
				outvalue=minval(dabs(T%ddata))
			case (4)
				outvalue=minval(abs(T%cdata))
			case (5)
				outvalue=minval(cdabs(T%zdata))
	...
	subroutine sum_TDatai(outvalue,T)
		...
		select case(T%ClassType)
			case (1)
				outvalue=sum(T%idata)
			case (2)
				outvalue=sum(T%sdata)
			case (3)
				outvalue=sum(T%ddata)
			case (4)
				outvalue=sum(T%cdata)
			case (5)
				outvalue=sum(T%zdata)
		end select
		return
	end subroutine
...
!**************************************************************************************************************	
	subroutine permutation_data3_inout(Res,index_not_permute,dimens)
	 ...
	 subroutine permutation_data3_fast_inout(Res,index_not_permute,dimens,Resi)
		integer,intent(in) ::   index_not_permute
		integer,intent(inout) :: Resi
		type(TData),intent(inout) :: Res(2)
		type(Dimension),intent(inout) ::dimens
		integer::dimen(3),lenD,classtype,nResi
		dimen=dimens
		nResi=3-Resi
		if( (nResi.le.0) .or. (nResi.ge.3))then
			call writemess('ERROR in permutation_data3_fast_inout')
			call error_stop()
		end if
			
		if(index_not_permute.eq.1) then
			lenD=dimen(1)
			select case (Res(Resi)%classType)
				case (1)
					call permutation_rank3_data1_int(Res(nResi)%idata,Res(Resi)%idata,dimen(1),dimen(2),dimen(3),lenD)
				case (2)
					call permutation_rank3_data1_real4(Res(nResi)%sdata,Res(Resi)%sdata,dimen(1),dimen(2),dimen(3),lenD)
				case (3)
					call permutation_rank3_data1_real8(Res(nResi)%ddata,Res(Resi)%ddata,dimen(1),dimen(2),dimen(3),lenD)
				case (4)
					call permutation_rank3_data1_com4(Res(nResi)%cdata,Res(Resi)%cdata,dimen(1),dimen(2),dimen(3),lenD)
				case (5)
					call permutation_rank3_data1_com8(Res(nResi)%zdata,Res(Resi)%zdata,dimen(1),dimen(2),dimen(3),lenD)
				case (6)
					call permutation_rank3_data1_logi(Res(nResi)%ldata,Res(Resi)%ldata,dimen(1),dimen(2),dimen(3),lenD)
				case (7)
					call permutation_rank3_data1_char(Res(nResi)%adata,Res(Resi)%adata,dimen(1),dimen(2),dimen(3),lenD)
			end select
			Resi=nResi
			return
		end if
		if(index_not_permute.eq.2) then
			...
			select case (Res(Resi)%classType)
				case (1)
					call permutation_rank3_data2_int(Res(nResi)%idata,Res(Resi)%idata,dimen(1),dimen(2),dimen(3),lenD)
				case (2)
					call permutation_rank3_data2_real4(Res(nResi)%sdata,Res(Resi)%sdata,dimen(1),dimen(2),dimen(3),lenD)
				case (3)
					call permutation_rank3_data2_real8(Res(nResi)%ddata,Res(Resi)%ddata,dimen(1),dimen(2),dimen(3),lenD)
				case (4)
					call permutation_rank3_data2_com4(Res(nResi)%cdata,Res(Resi)%cdata,dimen(1),dimen(2),dimen(3),lenD)
				case (5)
					call permutation_rank3_data2_com8(Res(nResi)%zdata,Res(Resi)%zdata,dimen(1),dimen(2),dimen(3),lenD)
				case (6)
					call permutation_rank3_data2_logi(Res(nResi)%ldata,Res(Resi)%ldata,dimen(1),dimen(2),dimen(3),lenD)
				case (7)
					call permutation_rank3_data2_char(Res(nResi)%adata,Res(Resi)%adata,dimen(1),dimen(2),dimen(3),lenD)
			end select
			Resi=nResi
			return
		end if
		if(index_not_permute.eq.3) then
			...
			select case (Res(Resi)%classType)
				case (1)
					call permutation_rank3_data3_int(Res(nResi)%idata,Res(Resi)%idata,dimen(1),dimen(2),dimen(3),lenD)
				case (2)
					call permutation_rank3_data3_real4(Res(nResi)%sdata,Res(Resi)%sdata,dimen(1),dimen(2),dimen(3),lenD)
				case (3)
					call permutation_rank3_data3_real8(Res(nResi)%ddata,Res(Resi)%ddata,dimen(1),dimen(2),dimen(3),lenD)
				case (4)
					call permutation_rank3_data3_com4(Res(nResi)%cdata,Res(Resi)%cdata,dimen(1),dimen(2),dimen(3),lenD)
				case (5)
					call permutation_rank3_data3_com8(Res(nResi)%zdata,Res(Resi)%zdata,dimen(1),dimen(2),dimen(3),lenD)
				case (6)
					call permutation_rank3_data3_logi(Res(nResi)%ldata,Res(Resi)%ldata,dimen(1),dimen(2),dimen(3),lenD)
				case (7)
					call permutation_rank3_data3_char(Res(nResi)%adata,Res(Resi)%adata,dimen(1),dimen(2),dimen(3),lenD)
			end select
			Resi=nResi
			return
		end if
	...
	subroutine permutation_data2_inout(Res,dimens)
		...
		subroutine permutation_data2_fast_inout(Res,dimens,Resi)
		type(TData),intent(inout) :: Res(2)
		type(Dimension),intent(inout) ::dimens
		integer::dimen(2),nResi
		integer,intent(inout) :: Resi
		if(dimens%Getrank().eq.1) then
			return
		end if
		...
		select case (Res(Resi)%classType)
			case (1)
				call permutation_rank2_data_int(Res(nResi)%idata,Res(Resi)%idata,dimen(1),dimen(2))
			case (2)
				call permutation_rank2_data_real4(Res(nResi)%sdata,Res(Resi)%sData,dimen(1),dimen(2))
			case (3)
				call permutation_rank2_data_real8(Res(nResi)%ddata,Res(Resi)%dData,dimen(1),dimen(2))
			case (4)
				call permutation_rank2_data_com4(Res(nResi)%cdata,Res(Resi)%cData,dimen(1),dimen(2))
			case (5)
				call permutation_rank2_data_com8(Res(nResi)%zdata,Res(Resi)%zData,dimen(1),dimen(2))
			case (6)
				call permutation_rank2_data_logi(Res(nResi)%ldata,Res(Resi)%lData,dimen(1),dimen(2))
			case (7)
				call permutation_rank2_data_char(Res(nResi)%adata,Res(Resi)%aData,dimen(1),dimen(2))
		end select
		Resi=nResi
		return
	end subroutine
...
!		dimen=DimDecomposeAll(dimen)
	...
	subroutine permutefo_data_fast_inout(Res,inde,dimen,Resi)
		type(TData),intent(inout) :: Res(2)
		type(Dimension),intent(inout) ::dimen
		integer,intent(inout)::Resi
		type(Dimension)::newdim
		...
		if(inde.eq.rank) then
			dimen=dimen%fuseIndex(1,rank-2)
			call permutation_data2_fast_inout(Res,dimen,Resi)
			dimen=newdim
			return
		end if
		num=inde-2
		...
		num=rank-3
		dimen=dimen%fuseIndex(3,num)
		call permutation_data3_fast_inout(Res,3,dimen,Resi)
		dimen=newdim
		return
	end subroutine
	subroutine permutefo_data(Res,T,inde,dimen,indimen)
		...
		if(inde.eq.rank) then
			dimen=indimen%fuseIndex(1,rank-2)
			call permutation_data2(Res,T,dimen)
			call Dimpermute_forwards(dimen,indimen,inde)
			return
		end if
		dimen=indimen.fuse.[1,inde-1]
		call dimen%fuse(3,dimen%Getrank())
	...
	subroutine permuteback_data_fast_inout(Res,inde,dimen,Resi)
		type(TData),intent(inout) :: Res(2)
		type(Dimension),intent(inout) ::dimen
		integer,intent(in)::inde
		integer,intent(inout)::Resi
		...
		num=rank-3
		dimen=dimen%fuseIndex(3,num)
		call permutation_data3_fast_inout(Res,1,dimen,Resi)
		dimen=newdim
		return
	end subroutine
	subroutine permuteback_data(Res,T,inde,dimen,indimen)
		...
		if(inde.eq.1) then
			dimen=indimen%fuseIndex(2,rank-2)
			call permutation_data2(Res,T,dimen)
			call Dimpermute_backwards(dimen,indimen,inde)
			return
		end if
		num=inde-2
		dimen=indimen%fuseIndex(1,num)
		num=rank-3
		dimen=dimen%fuseIndex(3,num)
