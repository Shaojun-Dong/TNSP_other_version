...
module Tensor_type
	...
	contains
		...
		generic,public::readData =>Tread_data,Tread_data2
...
!if RCONDM<0 keep the S(i)>0
		...
		procedure::ProductTensorRoutine1,ProductTensorRoutine2
		generic,public::ProductTensorRoutine=>ProductTensorRoutine1,ProductTensorRoutine2
...
!**************************************************************
	...
	type(TData),target,save::WorkingPermutation(2)!use permutation
...
!
!**********************************************************	
	public::Working_memory_report,Tensor_memory_length
contains
	...
	subroutine Tensor_memory_length(length)
		...
		call WorkingPermutation(1)%TData_length(leng)
		length=length+leng
		call WorkingPermutation(2)%TData_length(leng)
		length=length+leng
		call WorkingTensor1%TData_length(leng)
		length=length+leng
		call WorkingTensor2%TData_length(leng)
		length=length+leng
		length(1)=length(1)+Workingdimension1%outlenDimData()+&
...
!**************************************************************************************************************
	...
	subroutine Tread_data2(T,uni,datadimen)
		class(Tensor),intent(inout) :: T
		integer,intent(in)::uni,datadimen(:)
		CHARACTER(len=50)::notused
		...
		select case(T%getType())
			case(1)
				allocate(idata(dim1,dim2))
				do i=1,dim1
					read(uni,*)(idata(i,j),j=1,dim2)
				end do
				T=idata
			case(2)
				allocate(sdata(dim1,dim2))
				do i=1,dim1
					read(uni,*)(sdata(i,j),j=1,dim2)
				end do
				T=sdata
			case(3)
				allocate(ddata(dim1,dim2))
				do i=1,dim1
			...
			case(4)
				allocate(sdata(dim1,dim2))
				allocate(sdata2(dim1,dim2))
				do i=1,dim1
					read(uni,*)(sdata(i,j),j=1,dim2)
				end do
				do i=1,dim1
					read(uni,*)(sdata2(i,j),j=1,dim2)
				end do
				T=cmplx(sdata,sdata2,kind=4)
			case(5)
				allocate(ddata(dim1,dim2))
				allocate(ddata2(dim1,dim2))
				do i=1,dim1
					read(uni,*)(ddata(i,j),j=1,dim2)
				end do
				do i=1,dim1
					read(uni,*)(ddata2(i,j),j=1,dim2)
				end do
				T=dcmplx(ddata,ddata2)
			case(6)
				allocate(ldata(dim1,dim2))
				do i=1,dim1
					read(uni,*)(ldata(i,j),j=1,dim2)
				end do
				T=ldata
			case(7)
				allocate(adata(dim1,dim2))
				do i=1,dim1
...
!	  of T1 and the first diemsion of T2 should be equal.
	...
	subroutine ProductTensorRoutine1(Res,T1,T2,alpha,beta)! Res = alpha* T1 *  T2  + beta*C
...
!cccccccccccccccc permutation   cccccccccccccccccc
	type(Tensor) function permutation(T,newOrder)
		...
		call public_allocate_TData(WorkingPermutation(1),T%gettotalData(),T%getType(),.true.)
		call public_allocate_TData(WorkingPermutation(2),T%gettotalData(),T%getType(),.true.)
		WorkingPermutation(1)=T%TData
		call permutation%allocate(T)
		Resi=1
		do i=lenorder,1,-1
			call permutefo_data_fast_inout(WorkingPermutation,inde(i),permutation%TenDim,Resi)
			do j=1,i-1
		...
		permutation%TData=WorkingPermutation(Resi)
	...
	subroutine permutation_routine(T,newOrder)
		...
		Resi=1
		do i=lenorder,1,-1
			call permutefo_data_fast_inout(WorkingPermutation,inde(i),T%TenDim,Resi)
			do j=1,i-1
				if(inde(j).lt.inde(i))then
					inde(j)=inde(j)+1
				end if
		...
		T%TData=WorkingPermutation(Resi)
		call WorkingMemory%free()
		return
	end subroutine
...
!
	type(Tensor) function permutefo(T,inde)
		type(Tensor),intent(in)::T
		integer,intent(in)::inde
		call permutefo%allocate(T)
		call permutefo_data(permutefo%TData,T%TData,inde,permutefo%TenDim,T%TenDim)
		return
	end function
	...
	type(Tensor) function permutefo_name(T,indechar)result(permutefo)
		...
		if(long_Name_logi(indechar))then
			...
			Resi=1
			call permutefo%allocate(T)
			do inde=permutefo%getRank(),1,-1
				indexname=permutefo%outTensorName(i)
				if(indechar.equ.indexname)then
					call permutefo_data_fast_inout(WorkingPermutation,i,permutefo%TenDim,Resi)
				else
					i=i-1
				end if
			end do
			permutefo%TData=WorkingPermutation(Resi)
			return
		end if
	end function
	subroutine permutefo_name_routine(T,indechar)
		...
		if(long_Name_logi(indechar))then
			...
			do inde=T%getRank(),1,-1
				...
				if(indechar.equ.indexname)then
					call permutefo_data_fast_inout(WorkingPermutation,i,T%TenDim,Resi)
				else
					i=i-1
				end if
			end do
...
!
	type(Tensor) function permutefo_vec(T,vec_)
		...
		integer::lenVec,i,j,Resi
		integer,pointer::vec(:)
		if(.not.T%getflag())then
			call writemess("There is no data in the Tensor,(permutefo_vec)",-1)
			call error_stop()
		end if
		call WorkingMemory%check('permutefo_vec')
		lenVec=size(vec_)
		call WorkingMemory%get_memory(vec,lenVec,'permutefo_vec')
		vec=vec_
		call public_allocate_TData(WorkingPermutation(1),T%gettotalData(),T%getType(),.true.)
		call public_allocate_TData(WorkingPermutation(2),T%gettotalData(),T%getType(),.true.)
		WorkingPermutation(1)=T%TData
		Resi=1
		call permutefo_vec%allocate(T)
		do i=lenVec,1,-1
			call permutefo_data_fast_inout(WorkingPermutation,vec(i),permutefo_vec%TenDim,Resi)
			do j=1,i-1
		...
		permutefo_vec%TData=WorkingPermutation(Resi)
		call WorkingMemory%free()
		return
	end function
	subroutine permutefo_vec_routine(T,vec_)
		...
		Resi=1
		do i=lenVec,1,-1
			call permutefo_data_fast_inout(WorkingPermutation,vec(i),T%TenDim,Resi)
			do j=1,i-1
				if(vec(j).lt.vec(i))then
					vec(j)=vec(j)+1
				end if
...
!
	type(Tensor) function permuteback(T,inde)
		...
		call permuteback%allocate(T)
		call permuteback_data(permuteback%TData,T%TData,inde,permuteback%TenDim,T%TenDim)
		return
	end function
	...
	subroutine permuteback_name_routine(T,indechar)
		...
		if(long_Name_logi(indechar))then
			...
			do inde=1,T%getRank()
				indexname=T%outTensorName(i)
				if(indechar.equ.indexname)then
					call permuteback_data_fast_inout(WorkingPermutation,i,T%TenDim,Resi)
				else
					i=i+1
				end if
			end do
	...
	type(Tensor) function permuteback_name(T,indechar)result(permuteback)
		...
		if(long_Name_logi(indechar))then
			...
			call permuteback%allocate(T)
			i=1
			do inde=1,permuteback%getRank()
				indexname=permuteback%outTensorName(i)
				if(indechar.equ.indexname)then
					call permuteback_data_fast_inout(WorkingPermutation,i,permuteback%TenDim,Resi)
				else
					i=i+1
				end if
			end do
			permuteback%TData=WorkingPermutation(Resi)
			return
		end if
...
!
	type(Tensor) function permuteback_vec(T,vec_)
		...
		call public_allocate_TData(WorkingPermutation(2),T%gettotalData(),T%getType(),.true.)
		WorkingPermutation(1)=T%TData
		call permuteback_vec%allocate(T)
		Resi=1
		do i=1,lenVec
			call permuteback_data_fast_inout(WorkingPermutation,vec(i),permuteback_vec%TenDim,Resi)
			do j=lenVec,i+1,-1
		...
		permuteback_vec%TData=WorkingPermutation(Resi)
		call WorkingMemory%free()
		return
	end function
	...
	subroutine permuteback_vec_routine(T,vec_)
		...
		Resi=1
		do i=1,lenVec
			call permuteback_data_fast_inout(WorkingPermutation,vec(i),T%TenDim,Resi)
			do j=lenVec,i+1,-1
				if(vec(j).gt.vec(i))then
					vec(j)=vec(j)-1
				end if
...
!******************  contract the same names  *********************	
	...
	subroutine contract_name_routine(T,T1,name1,T2,name2,len_of_contract) 
		...
		class(Tensor),target,intent(inout) :: T1,T2
		character(len=*),intent(in)::name1(:),name2(:)
		integer :: i1(size(name1)),i2(size(name2))
		integer,optional,intent(in)::len_of_contract(2)
		integer::leni1,leni2,rank1,rank2
	...
	subroutine contract_name_routine1(T,name1,T2,name2,len_of_contract) 
		class(Tensor),target,intent(inout)::T
		class(Tensor),target,intent(inout) :: T2
		character(len=*),intent(in)::name1(:),name2(:)
		integer :: i1(size(name1)),i2(size(name2))
		integer,optional,intent(in)::len_of_contract(2)
		integer::leni1,leni2,rank1,rank2
	...
	subroutine contract_name_routine2(T,T1,name1,name2,len_of_contract) 
		class(Tensor),target,intent(inout)::T
		class(Tensor),target,intent(inout) :: T1
		character(len=*),intent(in)::name1(:),name2(:)
		integer :: i1(size(name1)),i2(size(name2))
		integer,optional,intent(in)::len_of_contract(2)
		integer::leni1,leni2,rank1,rank2
