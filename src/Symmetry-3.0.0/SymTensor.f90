...
module SymTensor_type
	...
	contains
		...
		procedure,public::SVDSymTensorRoutine=>SVDRoutine
		...
		generic,public::pointer=>Tpointer,Tpointer2,Tpointer3
		procedure::Tpointer,Tpointer2,Tpointer3
...
!**************************************************************************
	...
	subroutine ProductTensorRoutine(Res,T1,T2)
		class(SymTensor),intent(inout) :: Res
		class(SymTensor),intent(in) :: T1,T2
...
!Do not keep the Tesnor name
	function SVD(T,QuanNumCut)
		...
		if(present(QuanNumCut))then
			...
			Q2=T%getMaxQN(2)
			if(Q1.lt.Q2)then
				NewQuanNum=T%QuantumNumber(1)
			else
				NewQuanNum=T%QuantumNumber(2)
			end if
		end if
		quanNum1=T%QuantumNumber(1)
		quanNum2=T%QuantumNumber(2)
		do i=1,NewQuanNum%getQNlength()
			degcut=NewQuanNum%getDeg(i)
			Tdim1=quanNum1%getDeg(i)
			if(degcut.gt.Tdim1)then
				call NewQuanNum%setDeg(i,Tdim1)
				degcut=Tdim1
			end if
			Tdim2=quanNum2%getDeg(i)
			if(degcut.gt.Tdim2)call NewQuanNum%setDeg(i,Tdim2)
		end do
...
!********************************************************************
	subroutine QRdecomposition(T,Q,R) 
		...
		do i=1,NewQuanNum%getQNlength()
			...
			deg1=quanNum1%getDeg(i)
			if(deg.gt.deg1)then
				call NewQuanNum%setDeg(i,deg1)
				deg=deg1
			end if
			deg2=quanNum2%getDeg(i)
			if(deg.gt.deg2)call NewQuanNum%setDeg(i,deg2)
		end do
		classtype=T%getType()
