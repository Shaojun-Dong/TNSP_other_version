...
contains
	...
	logical function PRule4(dimen,indices,length)
...
!outQ=Q1+Q2
	...
	type(QuanNum) function ParityfuseOrder(order,Q1,Q2,newRule_)
		type(QuanNum),intent(in)::Q1,Q2
		type(Tensor),intent(inout)::order
		integer,intent(in)::newRule_
		call QuanFuseOrder(order,ParityfuseOrder,Q1,Q2)
		return
	end function
	...
	type(Tensor) function ParityNonZeroElement(dimen)
		type(SymDimension),intent(in)::dimen
		call ParityNonZeroElementParity(ParityNonZeroElement,dimen)
		if(ParityNonZeroElement%getRank().eq.1)then
			call ParityNonZeroElement%resetDim((/1,ParityNonZeroElement%getTotalData()/))
		end if
		return
	end function
	...
	integer function pParity1(dimen,ith,jth)result(Parity1)
		type(SymDimension),intent(in)::dimen
	...
	logical function ifParityIndex4(dimen,vec,ith,jth,rank)
	...
	subroutine reverseParityRule(T)
