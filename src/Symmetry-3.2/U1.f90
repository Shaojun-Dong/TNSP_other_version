...
contains
	...
	logical function U1Rule4(dimen,indices,length)
		type(SymDimension),intent(in)::dimen
...
!outQ=Q1+Q2
	type(QuanNum) function U1fuseOrder(order,Q1,Q2,newRule_)
		...
		select case(flag)
			case (0)
				...
				call QuanFuseOrder(order,U1fuseOrder,Q1,Q2,newRule)
			case (1)
				...
				call QuanFuseOrder(order,U1fuseOrder,nonZeroQ1,Q2,newRule)
			case (10)
				meanQN1=real(Q1%getmaxQN()+Q1%getminQN())/2.
				...
				call QuanFuseOrder(order,U1fuseOrder,Q1,nonZeroQ2,newRule)
			case (11)
				meanQN1=real(nonZeroQ1%getmaxQN()+nonZeroQ1%getminQN())/2.
				meanQN2=real(nonZeroQ2%getmaxQN()+nonZeroQ2%getminQN())/2.
				if(meanQN1.ge.meanQN2)then
				...
				call QuanFuseOrder(order,U1fuseOrder,nonZeroQ1,nonZeroQ2,newRule)
		end select
		return
	end function
	...
	type(Tensor) function U1NonZeroElement(dimen)
		type(SymDimension),intent(in)::dimen
		call U1NonZeroElementU1(U1NonZeroElement,dimen)
		if(U1NonZeroElement%getRank().eq.1)then
			call U1NonZeroElement%resetDim((/1,U1NonZeroElement%getTotalData()/))
		end if
		return
	end function
	...
	integer function U1Parity1(dimen,ith,jth)
		type(SymDimension),intent(in)::dimen
	...
	logical function U1ifParity4(dimen,vec,ith,jth,rank)
		type(SymDimension),intent(in)::dimen
	...
	subroutine reverseU1Rule(T)
		...
		if(.not.T%out_sample_flag())then
			call writemess('ERROR in  reverseU1Rule')
			call error_stop 
		end if
		call T%pointer(p)
		m=T%dim(1)
